...
- [ ] 4. `liftGroups.ts` — 群・開始・連鎖・先頭・釜ごとの提案・釜の組（**足すだけ**・旧経路は残す）
  - [ ] 4.1 `queueDisplay.ts`：`QueueSuggestion` に `serveAt` を足し、`boilSecondsOf` を export（`suggestionTiming` はまだ消さない——利用側の撤去は task 5 で一括）
  ...
  - [ ] 4.6 チェックポイント（新規モジュールとその検査だけが増え、既存はすべて通る）
