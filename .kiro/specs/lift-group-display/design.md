...
> **改訂（`lift-order-numbering` Component 3・2026-09-08）:** 麺種バッジ（`NoodleBadge`）の marker から点滅ドット `"boiling"` を撤去し、走行中は**上がり順の番号**を出す（`BadgeMarker = "none" | "ready" | "last" | { kind: "order"; n }`）。番号は `src/domain/lift-order.ts` の `liftOrderOf(view.timers, corrected)`——「同じ実効 endTime かつ同じ注文」を 1 単位（`Lift_Unit`）とした密な順位を**店舗全体**（担当外・未確定の Timer を含む `view.timers` 全件）で振る純粋関数——を `slotDisplay` が running に `liftOrder` として載せたもの。番号は点滅しない濃色の `tabular-nums` な文字（`aria-hidden`）で、aria-label の接頭辞は `Boiling {n}: ` / `Ready: ` / `Last: `。あわせて `NoodleBadge` の prop を `noodleType` → `label` に改め、語は呼び出し側が組む——走行中・茹で上がりは `display.orderItem` が在れば `displayName(orderItem)`（名の定義は `suggestionOf` の提案と共有・`displayName` の 1 箇所）、卓が在れば ` · {id}`、無ければ従来どおり麺種。**卓を数だけにするのは釜のバッジに限る**（バッジは品名と麺量で既に長く、前に番号のチップも付く）——提案の語とレールは `Table {n}` のままで、読み上げの語（accessible name）も `Table {n}` を残す（`NoodleBadge` の `spoken`・`lift-order-numbering` design の改訂 2）。残滓は麺種と ✓ のまま。群・先頭・連鎖・提案の描き分け（Component 3・本節の `SuggestionView`）は変えない（性質 3.6）。
