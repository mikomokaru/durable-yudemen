...
- **上限以下なら入力と同じ配列を返す**（AC 1.3）。`liveOrders` / `pendingOrders` と同じ理由で、参照同値を保って空振りの差分を作らない。**この即時脱出が効くのは上限に達するまでの間だけである**——集合が満杯（4096 件）になった後は、新規品目を含むあらゆる到着が上限を超えさせるので、**到着のたびに整列を払う**。ここを「定常状態は O(1)」と読んではならない。
...
- **費用。** 超過分 k は一つの到着の品目数（数件）に留まる——`upsertOrder` の入口の `items` は既に上限以下だからである（Component 2 の帰結）。しかし **k が小さくても整列は 4097 件に対して走る**ので、満杯の店では到着ごとに `O(n log n)` を払う。k 件の部分選択（`O(n)`）に置き換える余地はあるが、全順序で並べたほうが「落ちる集合が入力順に依らない」ことを読み取りやすい。**この選択は実測で確かめる**——満杯の状態へ単一店舗で 1000 Record を投入し、1 回あたりの費用が受理の応答時間に対して無視できることをtask 1 のチェックポイントで測る（無視できなければ部分選択へ替える）。
...
- 忘れられた注文の後着 → 引き継ぐ起点が無いので新しい `arrivalTime` で入り直す。**保証しない**（Requirement 3 AC 4 の但し書き。4096 件先の後着は現実の運用に無い）。
