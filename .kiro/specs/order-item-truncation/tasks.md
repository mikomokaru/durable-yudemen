...
- [x] 5. 忘れることの帰結（新しいコードは無い・既存の経路に落ちることの固定）
  ...
  - [x] 5.2 `tests/core/order-expiry-independence` と同形：性質 11（走行中の自立）。品目を忘れても走行中 Timer の集合・実効 endTime・Alarm・`tableMembers`・Boil_Sync の結果は変わらない
    - 実測（2026-09-08）：`tests/core/order-item-forgotten.property.test.ts` に 1 件（300 runs・3 回再実行とも通過）。`pending-order-expiry` 性質 5.9 と同じ形——Timer・設定・`now`・操作を固定し、**最も古い k 件を落とした**二つの状態を比べる。落とす k 件に**走行中の参照先を必ず含める**（含めなければ「参照が解けない」場面を踏まない）ことをproperty の中で前提として検証している。比べるのは Timer 集合・実効 endTime・`tableMembers`・Alarm 効果・`Persist` に載る Timer。**除く操作は `StartOrderItem` ただ一つ**——`Start`（アドホック開始）・`Adjust`（調整）・`RecordsReceived`（新規・0 件の除去）・`PlanArrived` を含む 10 種を振る（当初は 6 種しか生成しておらず、ヘッダが挙げた「調整」を検査できていなかった・レビュー指摘）。`PlanArrived` は**落ちた品目を指す**計画にして、`full` 側では計画対象と一致しうるが `forgotten` 側では必ず `isStale` で棄却される**非対称な経路**を踏ませる——それでも走行中 Timer は等しい（Timer は `synchronize` 済みで、採用側の再同期が no-op になる）。
