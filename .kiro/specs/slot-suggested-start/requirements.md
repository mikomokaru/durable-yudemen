...
4. **釜ごとの提案は startAt 最小の 1 件。** 当該釜を `slotIds` に含む推奨のうち最早の `startAt` を持つものを「次」として導出する（観測事実 8）。複数釜にまたがる推奨は含まれる各釜のカードに現れ、どのカードから押しても推奨の `slotIds` 全体で開始する。**（2026-09-05 追記）`lift-group-display` が改めた**——釜ごとの最早 1 件ではなく、店舗全体で次に上げる群（同じ卓で `serveAt` が等しい推奨の束・最早の群とその前がすべて始まった群）の品目を、当該釜を含み全釜が idle のものだけ、複数でも並べて出す。押せるのは群の先頭だけである（同 spec 判断 1・15〜17・19）。「各釜のカードに現れ、`slotIds` 全体で開始する」は保つ。
5. **時期は相対で示し、固定文言は英語にする。** `startAt` と補正済み現在時刻の差から `now`（`startAt ≤ now`）または `in mm:ss` を導出する。壁時計 `HH:MM` は用いない。`startAt` 到来前でも押せる（AC 8.2 / 8.3 を変えない）。卓は `Table {n}`。**（2026-09-05 追記）`lift-group-display` が改めた**——時期の語は消え、可視の語は空か `now`（群の先頭かつ `startAt` 到来）の 2 つだけになった。`in mm:ss` は描かない。到来までの時期は薄い塗り（`startAt` の 60 秒前・Prep_Lead から）と濃い塗り（到来）の相で伝える（同 spec 判断 3・6・AC 2.5・6.4）。壁時計を用いない・固定文言は英語・到来前でも押せる、は保つ。カードの固定文言を英語にするのは、#24（`pending-order-list-left-rail` 要件 3.9）がレールの固定文言を英語に固定し、カードの操作ラベルも `Start` / `Cancel` / `Complete` であるためで、日本語の固定文言はカードにも置かない。調理母語（硬さ）だけが `FIRMNESS_LABEL` 経由で入る。
...
2. THE Next_Suggestion SHALL 当該釜を `slotIds` に含む Cook_Recommendation のうち `startAt` が最小のもの 1 件から導出される **（2026-09-05 追記：`lift-group-display` Requirement 1・2 が改めた。当該釜の提案は「釜ごとに最早の 1 件」ではなく、店舗全体で表示できる群（Visible_Groups）の品目のうち当該釜を含み全釜が idle で Prep_Lead が来たもの全部で、`readonly SlotSuggestion[]` として複数並ぶ。押せるのは群の先頭だけ）**
...
3. THE Slot_Card SHALL Suggested_Start の下のラベルに商品名（Item_Name と Size_Name）・茹で加減・卓（`Table {n}`）・時期をこの順で表示し、行数を固定しない **（2026-09-05 追記：`lift-group-display` AC 3.3 が改めた。順は保つが、時期の位置に来る語は `now`（群の先頭かつ `startAt` 到来）だけで、それ以外は何も置かない。先頭でない品目はラベルだけで丸ボタンを持たない）**
4. THE Slot_Card SHALL 時期を Corrected_Now と `startAt` の差から導出し、`startAt ≤ Corrected_Now` なら `now`、それ以外なら残りを `in mm:ss` で表示する。固定文言は英語とし、日本語の固定文言を置かない（判断 5） **（2026-09-05 追記：`lift-group-display` AC 2.5・6.4 が改めた。`in mm:ss` は描かず、可視の語は空か `now` の 2 つだけ。`startAt` の 60 秒前（Prep_Lead）から薄く現れ、到来で濃くなる。「固定文言は英語」は保つ）**
5. THE Slot_Card SHALL 時期を壁時計（`HH:MM`）で表示しない **（2026-09-05 追記：`lift-group-display` AC 6.4 が強めた。壁時計に限らず時刻の語を一切持たない）**
