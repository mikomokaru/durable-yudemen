...
- [ ] 2. engine：状態・遷移・後着・永続
  - [ ] 2.1 `TimerState.orderItems`（旧 `pendingOrders`）、`snapshot.ts`、`migrate.ts` v13（`pendingOrders` → `orderItems`・`completedAt` / `interruptedAt` の欠如は null・**注文品目が不正なら既存どおり `MigrationFailed`**）、`types.ts` 版、`docs/persisted-schema-rollback.md` v13 行（切戻しの注意：v13 で残した cooking / done が v12 では未着手として現れる）
  ...
  - [ ] 2.3 `pending.ts`：`upsertOrder(items, timers, arrival)` / `removeOrder(items, timers, externalOrderId)` に **Timer 集合を渡し**（欠落品目の unstarted / cooking は `itemStatusOf` で判定）、「注文属性だけ更新・厨房の事実と生きた Timer を保持・arrivalTime 引継ぎ・後着に無い品目は unstarted だけ除く」に。`isSamePending` → `isSameOrderItems`
