...
18. **通信断（degraded）では左レールを列挙しない（レビュー P2・2026-09-08）。** 未調理は「自分を指す生きた Timer が無い品目」の導出なので、通信断中にローカルで Timer だけを消す完了（boiled の LocalComplete・早め上げは走行中への complete → LocalComplete）は品目に `completedAt` を書けず、調理済みの品目が未調理として左レールへ戻って見える（実測：[] → ["A"]・重複調理につながる表示）。サーバ未確定の `completedAt` を client で書くのは避け、ラジアル（`lift-group-display` AC 2.13）と同じく degraded では左レールも列挙せず、**通信復旧（pong で connectivity が up）ではなく、snapshot / Reconcile による再整合の完了まで**非表示にする（レビュー再現：snapshot より先に pong が届くと、古い品目集合とローカルで消した Timer 集合から未調理を導いて完了した品目が再表示された。既存の `sync` は通信断でも synced のままなので条件にできず、`ClientView.awaitingResync`——down で true・再整合で false・起動時 false（品目集合が空で再発しない）——を置く）。
...
7. WHILE client が degraded（通信断）のとき、または通信復旧の後でまだ snapshot / Reconcile で再整合していないとき（`awaitingResync`）、THE 左レール SHALL 品目を列挙しない（ラジアルと同じ）。再整合で復帰する。client は `completedAt` を書かない
...
11. **通信断の表示**：注文参照のある Timer について、通信断中の通常完了（boiled の LocalComplete）・早め上げ（走行中への complete → LocalComplete）・中断（LocalCancel）の後も左レールに品目が戻らず、pong で up になっただけでも戻らず、snapshot / Reconcile の再整合でサーバが確定した完了は出ず未調理だけが戻る（down → ローカル完了 → up → 再整合の各段階で検査）
...
| `ClientView.awaitingResync` | `src/client/connection.ts` | 通信復旧後、snapshot / Reconcile で再整合するまで真（左レールを列挙しない条件） |
