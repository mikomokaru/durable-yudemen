...
20. **走行中の状況を計画に考慮する——上げ窓（Lift_Window）を持ち、arms はソフト・arms + 手伝い（定数 2）はハードにする（実機の差し戻し・2026-09-06）。** #30・#31 の後も「次の品目は常に now」だった。計画が走行中を読むのは釜の占有・同じ卓の揃え先・同卓の同時上げの減点（1 本 1 秒相当）だけで、店舗全体の走行中が**いつ上がるか**を新しい品目の置き場所に反映していない。腕は arms 本で、上げて湯を切って盛り付けるまでに時間が要るのに、9 本が 1 分以内に上がる計画が出て、後の 7 本は茹で過ぎになる。Boil_Sync の maximin は ±h_i の内側でしか散らせず（60 秒の麺なら 12 秒幅）、上げの間隔にはならない。**上げと上げの間に要る秒数は設定にも定数にも無かった。**
    - **`liftIntervalSeconds`** を StoreConfig に足す（arms 本を上げて次を上げられるまでの秒数・現場で測る物理の値・既定 20 秒・妥当域 5〜120）。新しい設定であり、既存の許容幅の転用ではない。
    ...
    - **arms はソフト、arms + HELPER_ARMS はハード（ユーザー確定）。** 腕は 2 本でも「ちょっと手伝って」で一時的に増やせる——4 人家族の上げを同時にやりたいときがそれで、計画がそれを禁じる筋はない。ただし手伝えるのは物理的に 1 人なので、増やせるのは 2 本まで。窓の本数が arms を超えた分は**手伝いを頼む費用**として減点し（1 本あたり `liftIntervalSeconds` 秒相当・手伝いが無ければその 1 本は次の窓まで待つ、という時間の等価。新しい重みは足さない）、arms + 2 を超える計画は成立しない（ハード制約 (f)）。HELPER_ARMS = 2 は domain の定数（設定にしない・店舗差が実在するまで）。
    - **配置**：新しい品目の serveAt は、合流の規則（判断 18）で決めた候補（earliest か合流先の走行中）から、arms + 2 の窓を満たす最初の時刻へ後ろに動かす。同じ卓の群は「窓に arms 本ずつ」を基本に、arms を超えて arms + 2 までを同じ窓に載せるかは、その卓の遅れ（Table_Lag × w_table）と手伝いの費用の比較で決める（局所・決定的）。4 人家族・arms 2 なら 2+2 に分けると卓の遅れ 20 秒 × 2 本 × 2 に待ち 40 で 120、4 本同時なら超過 2 本 × 20 = 40 で、同時に上げる方を置く。9 本なら窓あたり 4 本が上限で 4・4・1 の 3 窓に割れ、「全部 now」は消える。釜が空いていても窓が埋まっていれば startAt は未来になり、提案は薄く現れて時刻が来たら濃くなる。計画は開始のたびに実際の走行中の上がり時刻から窓を引き直す。
...
8. **上げ窓の上限** — 任意の確定計画について、店舗全体の上がる時刻（走行中の実効 endTime と計画済みの serveAt）を並べたとき、長さ `liftIntervalSeconds` の任意の窓に上がる品目の slotSpan の合計は arms + 2 を超えない。走行中だけで既に超えている窓（開始後の事実）は検査の対象外とし、計画済みの serveAt はその窓を避けて置かれる
9. **上げの間隔** — 同じ卓の同じ茹で時間の品目 9 本・6 釜・arms 2・間隔 20 秒で、確定計画の serveAt は窓あたり 4 本（arms + 2）までで 20 秒以上離れた窓に並び、「全部 now」にならない（実機の再現）。4 人家族（同じ卓 4 本・arms 2）は同じ窓に 4 本載る（手伝いの費用 40 < 分けた卓の遅れ 120）
...
3. THE 計画 SHALL 店舗全体の「上がる時刻」（走行中の実効 endTime と計画済みの `serveAt`）を並べ、長さ `liftIntervalSeconds` の任意の窓に上がる品目の `slotSpan` の合計が `arms + HELPER_ARMS` を超えないよう、新しい品目の `serveAt` を候補（判断 18 の合流の規則で決めた時刻）以降の最初の窓へ置く（ハード制約 (f)）
4. THE 目的関数 SHALL Lift_Overflow の項を持つ——各窓で `arms` を超えた本数 × `liftIntervalSeconds`（秒相当・手伝いを頼む費用）。既存の Arms_Overflow 項を置き換える
5. THE 自前解 SHALL 同じ卓の群について、`arms` を超えて `arms + HELPER_ARMS` までを同じ窓に載せるかを、その卓の遅れ（Table_Lag × w_table と待ち）と Lift_Overflow の比較で決める（局所・決定的。同点は分けない側）
6. THE Acceptance_Gate SHALL 上げ窓の上限 `arms + HELPER_ARMS` を超える外部計画を feasible と認めない（ハード制約 (f)。走行中の上がり時刻と、計画順に見た手前の配置の `serveAt` で窓を埋めた上で判定する）。`arms` の超過は採点に委ねる
7. THE 計画 SHALL 窓で `serveAt` が後ろへ動いても合流の所属（`group` / `anchor`）を変えない。`anchor` は合流先の走行中の実効 endTime のまま
8. THE 計画 SHALL 大盛（`slotSpan` 2）を 2 本分として数える（上げる腕は釜の数だけ要る）
9. THE 変更 SHALL Boil_Sync を変えない
10. THE 確定計画の合成 SHALL 採用済み一片の `serveAt` を上げ窓の表に載せ、尾部はそれを避けて置く。採用済み一片が現在の走行中と窓の上限で衝突するときは、`keepsAnchor` と同じく陳腐化と見なして切る
