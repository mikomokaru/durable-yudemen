...
10. THE Acceptance_Gate と確定計画の合成 SHALL 合流の判定を `Placement.anchor` で行う——(a) `anchor` は現在の走行中の仲間の実効 endTime のいずれかに等しい、(b) `serveAt ≥ anchor − h_i`（手前に散らさない）、(c) **窓を当てる前に合流できた**——その配置が使う釜が、当該一片を置く前の解放表で `anchor + h_i − 茹で時間` までに空いていた（`earliest ≤ anchor + h_i`・判断 18 の合流の条件そのもの）。(c) が無ければ合流不能の品目が `anchor` を名乗り、client が未開始の群を開始済みと読む。押し出し（(e)）は **窓を当てる前に合流できた品目に限って**判定する——合流していない配置のうち、合流分で進めた解放表の下でいずれかの仲間に合流できた（`earliest ≤ 最遅の仲間 + h_i`）ものが、その合流の候補時刻（判断 18 の `joinedServeAt`）から `firstFit`（釜と上げ窓の両方）で得た最早の時刻より後ろに置かれていれば押し出し。合流できない品目どうしを後の batch で揃える配置（仲間 60 秒・残りが茹で 300 秒と 600 秒で両方を 600 秒に揃える）は押し出しではない
