...
20. **走行中の状況を計画に考慮する——上げ窓（Lift_Window）をハード制約にする（実機の差し戻し・2026-09-06）。** #30・#31 の後も「次の品目は常に now」だった。計画が走行中を読むのは釜の占有・同じ卓の揃え先・同卓の同時上げの減点（1 本 1 秒相当）だけで、店舗全体の走行中が**いつ上がるか**を新しい品目の置き場所に反映していない。腕は arms 本で、上げて湯を切って盛り付けるまでに時間が要るのに、9 本が 1 分以内に上がる計画が出て、後の 7 本は茹で過ぎになる。Boil_Sync の maximin は ±h_i の内側でしか散らせず（60 秒の麺なら 12 秒幅）、上げの間隔にはならない。**上げと上げの間に要る秒数は設定にも定数にも無かった。**
    - **`liftIntervalSeconds`** を StoreConfig に足す（arms 本を上げて次を上げられるまでの秒数・現場で測る物理の値・既定 20 秒・妥当域 5〜120 秒）。新しい設定であり、既存の許容幅の転用ではない。
    - **Lift_Window**：店舗全体で「上がる時刻」（走行中の実効 endTime と計画済みの serveAt）を並べ、任意の長さ `liftIntervalSeconds` の窓に上がる**品目の slotSpan の合計**が arms を超えないこと。大盛（slotSpan 2）は 2 本分の腕を使う（ユーザー確定）。数える範囲は店舗全体（arms は店舗の設定）。
    - **配置**：新しい品目の serveAt は、合流の規則（判断 18）で決めた候補（earliest か合流先の走行中）から、Lift_Window を満たす最初の時刻へ後ろに動かす。釜が空いていても窓が埋まっていれば startAt は未来になり、提案は薄く現れて時刻が来たら濃くなる。計画は開始のたびに実際の走行中の上がり時刻から窓を引き直す。
    - **ハード制約**：ゲートは外部計画にも同じ窓を検査する（釜の解放表と同じ資格の「上げ窓の表」・ハード制約 (f)）。ADR-0002「計画では arms はソフト（実質タイブレーク）」を覆す。目的関数の Arms_Overflow 項は役目を失うので外す（重みの導出 `max(0, w_table − 1)` も消える）。
    - **群と合流は残る**（判断 18・19）。錨は群の所属のために残し、合流した品目の serveAt は錨そのものではなく錨の後の空いた窓になりうる。`anchor` は合流先の走行中の実効 endTime のまま運び、client の `started = anchor > now` は変えない。合流の判定（`earliest ≤ 錨 + h_i`）と窓の適用は、判定 → 窓の順で行う（窓で後ろへ動いても合流の所属は変わらない）。
    - **Boil_Sync は変えない。** 開始後の ±h_i の微調整のまま。上げの間隔を作るのは計画の役目である。
    - 「腕を釜と同じ資源として扱う」という言い方はユーザーの像と違った。ここで扱うのは「走行中の上がり時刻を避けて置く」であり、腕を占有する資源のモデルは立てない（上げ窓の表は上がり時刻の並びから毎回導く導出値）。
...
4. ~~THE 目的関数 SHALL Arms_Overflow の項を持ち、その重みを `max(0, tableSyncWeight − 1)` から導く（設定項目も定数も足さない）~~ **撤回（判断 20）**：上げ窓がハード制約になり、同時に上がる本数の減点は役目を失う。項と重みの導出を外す
...
3. THE Acceptance_Gate SHALL feasibility を釜の排他・解放表の整合・`serveAt = startAt + 茹で時間`・`slotSpan` の充足・始めたまとまりを崩さないこと（AC 1.11・ハード制約 (e)）・上げ窓の容量（AC 9.3・ハード制約 (f)）で判定し、Lift_Group の `serveAt` の一致を求めない
...
5. THE 変更 SHALL `StoreConfig` の項目と妥当域を増減・変更しない（許容幅の設定は残し、撤去候補として記録する）。**判断 20 の改訂**：`liftIntervalSeconds` を 1 項目足す（既定 20・妥当域 5〜120・整数秒）。config ワイヤ・registry の compose・`RequestPlan` が運ぶ
...
8. **上げ窓の容量** — 任意の確定計画について、店舗全体の上がる時刻（走行中の実効 endTime と計画済みの serveAt）を並べたとき、長さ `liftIntervalSeconds` の任意の窓に上がる品目の slotSpan の合計は arms を超えない。走行中だけで既に超えている窓（開始後の事実）は検査の対象外とし、計画済みの serveAt はその窓を避けて置かれる
9. **上げの間隔** — 同じ卓の同じ茹で時間の品目 9 本・6 釜・arms 2・間隔 20 秒で、確定計画の serveAt は 2 本ずつ 20 秒以上離れた窓に並び、`startAt` は now・now+20・now+40・… となる（実機の再現）
...
**User Story:** As a 厨房スタッフ, I want 腕で上げられる本数と間隔に合わせて次の投入が提案される, so that 一度に上がりすぎて茹で過ぎにならない。

#### Acceptance Criteria

1. THE `StoreConfig` SHALL `liftIntervalSeconds`（arms 本を上げて次を上げられるまでの秒数・整数・既定 20・妥当域 5〜120）を持ち、config ワイヤと `RequestPlan` が運ぶ。妥当域外・非整数は既定へ畳む（他の採点パラメータと同じ規律）
2. THE 計画 SHALL 店舗全体の「上がる時刻」（走行中の実効 endTime と計画済みの `serveAt`）を並べ、長さ `liftIntervalSeconds` の任意の窓に上がる品目の `slotSpan` の合計が `arms` を超えないよう、新しい品目の `serveAt` を候補（判断 18 の合流の規則で決めた時刻）以降の最初の窓へ置く（ハード制約）
3. THE Acceptance_Gate SHALL 上げ窓の容量を超える外部計画を feasible と認めない（**ハード制約 (f)**。走行中の上がり時刻と、計画順に見た手前の配置の `serveAt` で窓を埋めた上で判定する）
4. THE 計画 SHALL 窓で `serveAt` が後ろへ動いても合流の所属（`group` / `anchor`）を変えない。`anchor` は合流先の走行中の実効 endTime のまま
5. THE 計画 SHALL 大盛（`slotSpan` 2）を 2 本分として数える（上げる腕は釜の数だけ要る）
6. THE 変更 SHALL Boil_Sync を変えない
7. THE 変更 SHALL 目的関数から Arms_Overflow の項と `max(0, tableSyncWeight − 1)` の導出を外す（上げ窓がハード制約になるため）
8. THE 確定計画の合成 SHALL 採用済み一片の `serveAt` を上げ窓の表に載せ、尾部はそれを避けて置く。採用済み一片が現在の走行中と窓で衝突するときは、`keepsAnchor` と同じく陳腐化と見なして切る
