...
11. THE Acceptance_Gate SHALL 走行中の錨に合流できた品目を、合流できない品目のために錨より後ろへ押し出した計画を feasible と認めない（**ハード制約 (e)**。錨以下に提供する配置だけで解放表を進めた上で、錨より後ろの品目の `slotSpan` 個の釜が「錨 − 茹で時間」までに空いていたなら押し出し）。AC 1.9 は目的関数では守れない——最遅参照の卓同期項は「合流できない 1 本のために全員を遅らせる」配置を真に良いと採点する（レビューの実測：合流 4597 対 全員遅延 2887）——ので、feasibility の側に置く。自前解はこの述語を構成から満たす
...
8. THE 確定計画の合成 SHALL 採用済み一片を現在の実効錨で再検証する——走行中の錨が在る卓では、錨以下に提供する配置（合流分）が錨に**一致**し、かつ合流できる品目を錨より後ろへ押し出していないこと（`keepsAnchor`・Acceptance_Gate の (e) と同じ述語）。違反した一片以降を切り、残した接頭辞の解放表から尾部を再計算する。受領時の比較も同じ合成を通す。合流する部分集合は外部解に強制しない（残り容量が 1 品分で自前解が A を選んでも、外部解が B を合流させ A を後ろに置く一片は、A が B の後では合流できないので守っている）。錨が早まって本当に合流できなくなった品目の一片は正当な後続の batch として維持する
