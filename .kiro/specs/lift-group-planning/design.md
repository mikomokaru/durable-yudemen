...
| 設定（arms / toleranceRatio）の差し替えを跨いだ状態で外部計画が届く | 受領（plan.ts）は判定の前に `resynchronize`（settle と同じ再同期）を通し、採用後に確定する走行中と同じ実効 endTime＝同じ錨で採点する。棄却時は同期前の状態を返す（AC 6.6） | 判断 4・AC 7.1 |
