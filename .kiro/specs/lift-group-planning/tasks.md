...
- [ ] 21. 上げ窓（Lift_Window）——判断 20・ADR-0009・Requirement 9（design レビュー 6 件を反映済み）
  ...
  - [ ] 21.6 `keepsAnchor`：配置を単位（`anchor` 付きは同じ anchor・同じ serveAt の pack、無しは 1 品）にまとめ、(serveAt, startAt, 釜) 順に一つずつ解放表と上げ表へ載せながら検査。pack は 4 条件（成員に在る——(a) は 21.4 のレビュー追記で先に据えた・手前に散らさない・**手前の単位で進めた表で**各配置が合流できた・`serveAt` が候補の最大から **pack 全体の span** で firstFit した時刻に一致）、`anchor` 無しは「窓を当てる前に合流できた品目」に限り「候補時刻からの firstFit より後ろ」を押し出し。回帰 6 件：**自前解の pack を受け入れる**（走行中 3 本が 54・54・66 秒・残り 2 品の候補 60 秒 → 99 秒の pack が真）／走行中 4 本が 60 秒・残りを 105 秒に置く一片は守る／合流不能の品目の `{anchor: 60, serveAt: 600}` は棄却／仲間 60 秒・茹で 300 と 600 を 600 秒に揃える後続 batch は守る／空き 1 釜に Thin を [0,60]・[60,120] と置いて両方 `anchor: 60` は棄却（集合）／Thin と 600 秒の品目を両方 600 秒に置き Thin に `anchor: 60` は棄却（延期の理由）
