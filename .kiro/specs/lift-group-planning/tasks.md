...
- [ ] 21. 上げ窓（Lift_Window）——判断 20・ADR-0009・Requirement 9（design レビュー 6 件を反映済み）
  ...
  - [ ] 21.9 テスト：`schedule.example`（4 人家族は同時・9 本は 4・4・1・大盛は 2 本分・slotSpan 4 / arms 1 は配置されない）（21.5 で済）、`continuous-input.example` の期待値を「上限を満たす最初の時刻」へ（21.5 で済）、`admit.example`（上限超過の外部計画は棄却・arms 超過は採点・`anchor` の不正な主張は棄却）、`objective.*`、`digest.example`、`solver` の契約テスト
