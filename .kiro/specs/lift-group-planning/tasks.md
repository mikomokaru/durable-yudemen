...
- [ ] 21. 上げ窓（Lift_Window）——判断 20・ADR-0009・Requirement 9（design レビュー 6 件を反映済み）
  ...
  - [ ] 21.6 `keepsAnchor`：配置を startAt 順に一つずつ解放表と上げ表へ載せながら検査。`anchor` 付きは 4 条件（成員に在る・手前に散らさない・**手前の配置で進めた表で**合流できた・`serveAt` が候補時刻からの firstFit に一致）、`anchor` 無しは「窓を当てる前に合流できた品目」に限り「候補時刻からの firstFit より後ろ」を押し出し。回帰 5 件：走行中 4 本が 60 秒・残りを 105 秒に置く一片は守る／合流不能の品目の `{anchor: 60, serveAt: 600}` は棄却／仲間 60 秒・茹で 300 と 600 を 600 秒に揃える後続 batch は守る／空き 1 釜に Thin を [0,60]・[60,120] と置いて両方 `anchor: 60` は棄却（集合）／Thin と 600 秒の品目を両方 600 秒に置き Thin に `anchor: 60` は棄却（延期の理由）
