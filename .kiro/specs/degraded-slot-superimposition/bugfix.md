...
## 追記（ADR-0015・2026-09-14）

本文書が「破られている不変条件」と呼ぶ **1 スロット ≤ 1 タイマー**は、`ClientView` だけの性質ではない。
**server 側の正本（`TimerState.timers`）でも守られていなかった**——釜を鍵にした表ではなく配列であり、
開始の側にも占有の検査が無かった（`slot-suggested-start` AC 3.7）。

ADR-0015 で、`decide` に「遷移の結果が同じ釜を 2 つ以上の Timer に占められた状態になるなら拒否する」
という不変条件を置いた。本文書が記録する client 側の重ね合わせは**別の経路**であり、そちらは未修正で
ある（write-back 不在という既存スコープ外の問題と組み合わさっている）。

**同じ不変条件が 2 層で破れていた**ことを、両方の記録から辿れるようにするための追記である。
