...
stateDiagram-v2
  ...
  unlocking --> running: 無音 BufferSource の onended 発火で解錠成立を確認 → 解錠リスナ群を一括解除
...
- **unlocking** — 初回ジェスチャで AudioContext を生成し無音 warm-up を開始したが、まだ解錠成立を確認していない過渡状態。無音 BufferSource の `onended` が発火して初めて running へ移る。発火を確認できないうちは「鳴らしたとみなさない」。
 ...
 *     生成し無音 warm-up（要件4.1/4.2）。無音 BufferSource の onended 発火で解錠成立を確認してから running 化し、
...
- **Audio_Unlock（要件4.1 / 4.2 / 4.6）** — 初回ジェスチャ（`touchstart`/`touchend`/`click`/`keydown` を capture で待受）で AudioContext 生成 ＋ 無音 warm-up を行い、無音 BufferSource の `onended` 発火で解錠成立を確認してから running 化し、確認後に解錠リスナ群が一括解除されることを確認。warm-up 失敗（onended 未発火）→ 解錠未完了のまま次ジェスチャで再試行。
