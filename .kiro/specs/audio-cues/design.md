...
2. **計算と作用を分離し、クライアントへ徹底する** — 「鳴らすか否か」の判定（boiled 集合の導出・Pre_Alert 閾値クロスの検知・再生済みの判定・Done_Cue の周期到来判定）は **WS も DOM も時計も AudioContext も持たない決定的な純粋関数**に寄せる（`src/client/` 配下の純粋モジュール）。実際の音声出力（AudioContext の生成・warm-up・resume・再生）は**端の作用**として React フックへ隔離する（`src/client/components/`・既存 `useWakeLock` と同列）。
...
- iOS PWA における Audio_Unlock（初回ジェスチャでの解錠・warm-up）。
- Audio_Session の自己回復（resume・破棄と再生成・再 warm-up）。Done_Cue の 5 秒周期を自己修復リトライの機会として用いる。
...
- **自動再生制限** — 最初のユーザージェスチャを起点に解錠（warm-up）するまで、Pre_Alert_Cue / Done_Cue は鳴らない（要件4.3）。
...
- **resume 失敗（`InvalidStateError`）** — 一部 iOS で resume が失敗する。破棄・再生成＋再 warm-up で回復する（要件7.4）。
...
flowchart TB
  ...
  subgraph Edge["端の作用（src/client/components/・★新規・useWakeLock と同列）"]
    HOOK["★音声出力フック（useAudioCues）<br/>Audio_Session の生成/解錠/resume/再生成<br/>≤1s 評価ティック・Touch_Cue 再生口の提供"]
  end
  ...
  HOOK <-->|"生成・warm-up・resume・再生"| WEBAUDIO
...
- **音声出力フック（端の作用）** — Audio_Session（AudioContext）のライフサイクルを所有し、純粋判定の出力を受けて実際に音を鳴らす。Audio_Unlock（初回ジェスチャ）・resume・破棄と再生成・≤1s 評価ティック・Done_Cue の 5 秒周期・Touch_Cue の再生口を担う。`useWakeLock` の規律（可視時のみ・前面復帰のたびに取り直す・優雅な劣化）を手本にする。
...
### Audio_Session の状態機械（解錠・resume・再生成の遷移）

```mermaid
stateDiagram-v2
  [*] --> uninitialized: 起動 / リロード直後（AudioContext 未生成）
  uninitialized --> unlocking: 初回ジェスチャ → 生成 + 無音 warm-up 開始（Audio_Unlock・要件4.1/4.2）
  unlocking --> running: 無音 onended 発火 ＝or＝ resume 成功(state running) の早い方で解錠成立 → 解錠リスナ群を一括解除
  unlocking --> uninitialized: warm-up 失敗 → 次ジェスチャで再試行（要件4.6）
  uninitialized --> unsupported: 音声出力 API 非提供（要件4.5）

  running --> suspended: バックグラウンド / 画面ロック（要件7 suspended）
  running --> interrupted: 着信 / 他アプリ音声 / アラーム（要件7 interrupted）

  suspended --> running: 可視復帰 or ジェスチャ → resume 成功（要件7.2）
  interrupted --> running: 中断終了 → resume 成功（要件7.3）

  suspended --> closed: resume が InvalidStateError（要件7.4）
  interrupted --> closed: resume が InvalidStateError（要件7.4）
  running --> closed: OS による破棄

  closed --> running: 破棄して再生成 + 次ジェスチャで再 warm-up（要件7.4）

  unsupported --> [*]: 優雅に劣化（UI/Timer は継続・要件4.5）

  note right of unlocking
    warm-up（無音バッファ再生）を
    「試みた」だけで running を主張しない。
    無音 BufferSource の onended が発火し
    実際に再生が完了したのを確認してから
    running 化する（真＝状態について嘘をつかない）
  end note

  note right of running
    Done_Cue の 5 秒周期の冒頭で
    状態を確認し、suspended/interrupted
    なら resume を試みてから再生する
    （自己修復リトライ・要件7.5/7.6）
  end note
```

- **uninitialized** — AudioContext 未生成。Audio_Unlock 前。Pre_Alert / Done は鳴らない（要件4.3）。
- **unlocking** — 初回ジェスチャで AudioContext を生成し無音 warm-up を開始したが、まだ解錠成立を確認していない過渡状態。無音 BufferSource の `onended` 発火、または `resume()` 成功（state running）の**いずれか早い方**で running へ移る。どちらも「実際に再生可能になった」正直な証跡であり、warm-up を試みただけで running を主張しない。onended は iOS の一部で発火しないため、resume 成功も証跡に含める（onended のみに依存すると永久未解錠になりうる）。
- **running** — 再生可能。Touch / Pre_Alert / Done を鳴らせる。
- **suspended / interrupted** — プラットフォーム由来で停止。次の可視化・ジェスチャ・Done 周期冒頭で resume を試みる。
- **closed** — 再生不能。resume 失敗（`InvalidStateError`）または OS 破棄で至る。破棄して新 AudioContext を生成し、次ジェスチャで再 warm-up する。`ctx.sampleRate` には干渉しない（正常値は OS / 出力デバイス任せでデバイスごとに異なり〔macOS は 48000・古い iOS は 44100 など〕、特定値で弾くとその値でないデバイスを恒久的に無音化するため）。
- **unsupported** — `AudioContext` / `webkitAudioContext` を提供しない環境。何もせず劣化する（要件4.5）。

> **なぜ 5 秒周期を回復の機会に使うか（待つなら寝かせる、ではなく前面で鳴らす）:** Done_Cue は boiled が残る限り 5 秒ごとに必ず評価される。この既にある周期の冒頭で Audio_Session の状態を確認し、必要なら resume する——回復のためだけの別ループ（`setInterval` の追加）を持たず、既存の鳴動周期に回復を相乗りさせる。これが「再生失敗・中断からの自己修復リトライ」の正体である（要件7.5 / 7.6）。

> **解錠の定石（onended 確認と多イベント capture 待受・出典明記）:** iOS の解錠手法には広く知られた定石がある。warm-up（無音バッファ再生）後に「鳴らしたとみなす」のではなく、無音 BufferSource の `onended` コールバックが発火して**実際に再生が完了したことを確認してから**解錠完了（running）とする。確認できて初めて初回ジェスチャのリスナ群を外す。あわせて解錠リスナは `touchstart` / `touchend` / `click` / `keydown` を **capture フェーズ**（`addEventListener(..., true)`）で張り、onended 確認後に一括解除する（`pointerdown` 一本より初回ジェスチャを取りこぼしにくい）。この定石は本設計哲学「真＝状態について嘘をつかない」に合致する（warm-up を試みただけで running を主張しない）。
>
> これは [Howler.js](https://github.com/goldfire/howler.js)（MIT ライセンス）の `_unlockAudio` の発想（元アイデアは Paul Bakaus「Web Audio on iOS」）を**手法（パターン）として援用**したものであり、**コードの逐語コピーではない**（ライセンス配慮）。ライブラリ依存ではなく、広く知られた定石を最小実装で借りる位置づけであり、steering（YAGNI・構造の主権）に沿う。`tooling.md` の確定スタックに新規依存（howler 等）を加えるものではない。
 ...
 *
 * 担うのは作用の配線だけ（判定は audioCue.ts に委ねる）:
 *   - Audio_Unlock — 初回ジェスチャ（touchstart/touchend/click/keydown を capture フェーズで待受）で AudioContext を
 *     生成し無音 warm-up（要件4.1/4.2）。無音 BufferSource の onended 発火 or resume 成功(state running) の早い方で
 *     解錠成立を確認してから running 化し、
 *     確認後に解錠リスナ群を一括解除する（試みただけで解錠済みと主張しない）。失敗時は次ジェスチャで再試行（要件4.6）。
 *     非対応環境は何もしない（要件4.5）。
 ...
 *   - Pre_Alert 再生 — 解錠済みなら fire の各 timerId に対し Pre_Alert_Cue を 1 回鳴らす（要件2.1/2.8）。
 *   - Done_Cue 5 秒周期 — dueDoneCue が true の周期で、冒頭に Audio_Session 状態を確認し suspended/interrupted
 *     なら resume を試みた上で Done_Cue を 1 回鳴らし、lastRingAt を更新する（要件3.1/7.5/7.6）。
 *     boiled が空になれば lastRingAt を解除し、次に非空化したら 1 秒以内に最初の Done を鳴らす（要件3.3/3.4）。
 *   - 可視復帰 — visibilitychange で可視化を検知し、suspended/interrupted なら resume、boiled が残れば
 *     Done_Cue 周期を 1000ms 以内に再開する（要件5.2/5.3）。useWakeLock と同じ visibilitychange 規律。
 *   - 自己回復 — resume が InvalidStateError なら AudioContext を破棄し再生成、次ジェスチャで再 warm-up（要件7.4）。
 *     ctx.sampleRate には干渉しない（正常値はデバイス任せで一定でなく、特定値で弾くと無音化するため）。
 *
 * これらはすべてサーバ状態・状態遷移・ワイヤ表現を変えない（要件1.4/7.10）。失敗は握り潰さず次周期へ繰り越す（要件3.11）。
 */
...
| Audio_Unlock 失敗（warm-up 例外） | 初回ジェスチャの try/catch | エラー表示せず、解錠未完了のまま。次のジェスチャで再試行 | 4.6 |
...
| resume 失敗（`InvalidStateError` 等） | resume の catch | Audio_Session を破棄（close）→ 新規生成 → 次ジェスチャで再 warm-up | 7.4 |
...
入力で振る舞いが変わらない端の作用は、代表例 1〜3 で確認する（100 反復しない）。mock の `AudioContext`（`state` / `sampleRate` / `resume` / `close` / `createOscillator` / `createBuffer*` を差し替え）と擬似 `visibilitychange` を用いる。

- **Audio_Unlock（要件4.1 / 4.2 / 4.6）** — 初回ジェスチャ（`touchstart`/`touchend`/`click`/`keydown` を capture で待受）で AudioContext 生成 ＋ 無音 warm-up を行い、`onended` 発火または `resume()` 成功（state running）の早い方で解錠成立を確認してから running 化し、確認後に解錠リスナ群が一括解除されることを確認。両証跡とも得られず（resume 失敗）→ 破棄して次ジェスチャで再試行。
- **Touch_Cue レイテンシ／再トリガ（要件1.1 / 1.6）** — `playTouchCue` で短音再生が即時呼ばれ、連続呼び出しで都度新しいノードが生成される。
- **再生終了ノードの後始末（要件3.11）** — 各 Cue ノードの `onended` で `disconnect` され参照が解放されること、Done_Cue を複数周期回しても finished ノードが滞留せず再 start もされないことを mock ノードのスパイで確認。
- **Done_Cue 周期＋自己回復（要件3.1 / 7.5 / 7.6 / 3.11）** — boiled 非空で 5 秒周期ごとに再生。周期冒頭で state 確認 → suspended/interrupted なら resume → 再生の順。1 周期失敗させても次周期でリトライ。
- **resume 失敗 → 再生成（要件7.2 / 7.3 / 7.4）** — `resume` を `InvalidStateError` で失敗させ、close → 新規生成 → 次ジェスチャ warm-up を確認。sampleRate には干渉しないため、48000 等のデバイスでも解錠が成立することを確認（特定値で弾かない）。
- **可視復帰（要件5.1 / 5.2 / 5.3 / 5.9）** — `visibilitychange`（→visible）で suspended なら resume、boiled 残存なら Done_Cue 再開、boiled 空なら鳴らさない。Wake_Lock 不在でも再評価が動く。
