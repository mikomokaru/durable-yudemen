...
5. THE Shell_Instrumentation SHALL 計装を `src/shell/` の層に限定し、`src/engine/` の純粋関数を変更しない。
...
7. WHEN Correlator が二つのログ（Operation_Log と Instrumentation_Log）をまたぐ cross-source の意味的比較——instanceId 出現区間へのタイマー帰属、検証条件 a の `alarm` と `done` の順序、検証条件 b の wake 境界における active 件数の導出——を行う, THE Correlator SHALL 各 ServerMessage が保持する `serverTime` を比較時刻として用い、`serverTime` を持たない record（client 由来の `start`／`cancel` 送信など）に限りその record 自身の `at` へフォールバックする。これにより cross-source 比較は単一クロック（サーバクロック）上で行われ、クライアントとサーバのクロックスキューによる順序逆転の影響を受けない。なお本条は意味的比較の比較時刻のみを規定し、6.1 の汎用統合（記録された epoch-ms `at` による安定整列）は変更しない。
...
5. THE Observability_Harness SHALL `src/engine/` 配下の既存ファイルを追加・変更・削除せず、計装の追加を `src/shell/` 配下と CLI 側のファイルに限定する。
6. THE Observability_Harness SHALL `src/domain/messages.ts` に定義された既存の ClientMessage / ServerMessage のワイヤ形式のみに従ってメッセージを送受信し、新たなメッセージ種別やフィールドを導入しない。
...
- **時刻軸とクロックスキュー**: Operation_Log は Probe_Client（Node）のクロックで `at` を打刻し、Instrumentation_Log は Cloudflare サーバのクロックで `at` を打刻する。両者は異なるクロックであり実測で約180ms のスキューが観測された。したがって二つのログをまたぐ意味的比較（インスタンス区間帰属・検証条件 a の順序・検証条件 b の境界）は、ワイヤプロトコルが各 ServerMessage に載せる単一サーバクロックの `serverTime` に揃え、クロックスキューに起因する順序逆転を避ける（`serverTime` を持たない client 由来 record は自身の `at` にフォールバック）。これはライブパイロット観測で発見・確認した事項である。汎用統合（mergeByTime）は記録された `at` で整列し変更しない。
