...
### 改訂（2026-09-08・#40 のマージ後・ユーザー指示）

**番号を 2 段にする。** クラスタ（同じ実効 endTime＝一括で上がる集合）に上がる順の番号を振り、クラスタ内の注文に枝番を振る（`4a` / `4b`）。判断 2 の「同じ実効 endTime でも注文が違えば別の番号」は保つが、**別の数**ではなく**同じ数の別の枝**にする——以前の振り方では同時に上がる 2 つの注文が 2 と 3 という無関係な数に分かれ、番号から「同時に上がる」ことが読めなかった。クラスタ番号を共有して枝で分ければ、上がるタイミング（クラスタ）と盛り分けの単位（注文）を一つの表記が両方語る。

これに伴う改訂：

1. **クラスタ番号**は走行中の相異なる実効 endTime を昇順に並べた密な順位（店舗全体・1 始まり）。同じ Sync_Set の Timer は注文が違っても同じクラスタ番号を持つ。
2. **枝番**はクラスタ内の注文の密な順位（1 始まり）。並びは判断 2 のまま——枝内の最早 `startTime` の昇順、同値は `externalOrderId` の符号単位順。アドホックは 1 本で 1 つの枝。
3. **表記は `{クラスタ}{枝}`**（クラスタは十進、枝は小文字のアルファベット・`4a` / `4b` / `12c`）。枝が 26 を超えたら `aa`・`ab`（表計算の列と同じ規則）——実務では起きないが表記は全域で定める。
4. **単独のクラスタでも枝を出す**（`3a`・ユーザー確定 2026-09-08）。表記の形を揃え、後から別注文が同じ時刻に合流しても形が変わらない。
5. aria-label は `Boiling 4b: {語}`。判断 4（点滅しない・マーカーの置き換え）と判断 5（複数釜は 1 本）は変わらない。判断 7 の「二桁は想定するが三桁は想定しない」はクラスタ番号について読む。
6. 性質 3.1〜3.5 はクラスタと枝の語で読み直す（design の改訂と `lift-order.property` を正とする）。
...
1. ~~番号を店舗全体で振るか~~ → 店舗全体で振る（ユーザー確定・2026-09-08）。
2. ~~同じ実効 endTime を同じ番号にするか~~ → 同じ実効 endTime かつ同じ注文を同じ番号にする（ユーザー確定）。同じ実効 endTime の中の注文の順の断ち方（推奨：最早 `startTime` → `externalOrderId`）。
3. ~~未確定（provisional）の Timer を数えるか~~ → 数える（ユーザー確定・2026-09-08）。同じ実効 endTime の中の注文の順は最早 `startTime` → `externalOrderId`（同）。
