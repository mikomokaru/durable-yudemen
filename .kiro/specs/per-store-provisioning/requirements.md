...
5. THE Store_Registry_DO SHALL 合成結果が完全な `StoreConfig`（unitCount / arms / toleranceRatio / noodlePresets の全フィールド）であり、既存検証関数の値域に収まることを保証する。（追記・`lift-group-planning` 判断 20：主張対象でない採点パラメータ——重み・許容幅・距離・レイアウト——は合成が既定を供給し、既定より前に永続された投影の欠如は store DO が既定へ畳む。上げの間隔 `liftIntervalSeconds` は 2026-09-17 に店舗差が実在して主張対象へ上げた——数値フィールドとして unitCount / arms / toleranceRatio と同じ扱い）
