...
- **固定した「今」を残した再生成が計画順で成り立たないとき（判断 14）**：候補 K = 1 段目の配置の時刻をすべて保ち、配分した釜だけを交換した計画。**交換**とは、配分で「今」の品目が新しく取った釜（配分後にだけ在る釜・`taken`）を使っていた 1 段目の将来配置を、動いた「今」の品目が空けた釜（配分前にだけ在る釜・`freed`）へ index 順に写すこと（`swapNow`・配分は排他的で本数を保つので `|taken| = |freed|`）——「配分した釜に置き換えるだけ」では、空き釜へ動いた「今」の品目が同じ釜に 30 秒後で置かれていた将来配置と重なり K が違法になる。K は再生成も取り置きも無しに計画順の物理的なハード制約で検証する（`isLegal`・性質 4.8 の検査と同じ形）。K が合法なら、K と「固定を外して卓を再生成した計画」（loop を終えたもの・「今」の入れ替えを含む）を同じ旧 Shown_Plan に対する `scoreSchedule` の総費用で比べ、真に低い方（同点は K）。K が違法なら再生成。「今」の解除は物理的な例外ではなく、この比較の結果としてだけ起こる。比較は loop の外で一度だけ行う（K は品目を足さず、停止性は nowSet の増加のまま）。経過は `scheduleStages`（`CandidateStages = { stage1, regenerated, swap, completed }`）で公開し、性質 4.7′ が K を実際に見る。反例（6 釜・釜 0 boiled・arms 1・L 5・同卓の 45 秒麺 N と 75 秒麺 A・別注文の 45 秒麺 C と大盛 45 秒麺 M）を固定 Example にし、両候補の合法性と総費用を示す——実測では総費用の向きは重みで変わる：同期の重み 0 なら再生成（N 今・M 5 秒後）が 30 秒相当低く（1427 対 1457）、既定の重みでは A と N を 30 秒ずらす再生成が卓の遅れを払い K（N 30 秒後・M 今）が 30 秒相当低い（1457 対 1487）。
- 取り置き（手前の一片に対する「今」の釜の ∞）は現在の計画順の解放表を維持するための機構として残す（上がりは載せない）。5.6 のための取り置きではない。
- 再生成は固定した「今」の品目に残りを揃えない（判断 15）が、再生成した品目どうしは従来の規則（Property 1／16 を固定分を除いて主張）。
- `reanchor`（`retain`）：復元した配置の錨は「旧 Shown_Plan の品目のうち今回 Timer になったもの」の実効 endTime にだけ付け直す（`plan-stability` 判断 9 の補足）。
