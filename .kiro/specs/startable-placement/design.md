...
- **固定した「今」を残した再生成が計画順で成り立たないとき（判断 14）**：候補 K = 1 段目の配置の時刻をすべて保ち、配分した釜だけを交換した計画（`revalidate` を通す）。K が合法なら、K と「固定を外して卓を再生成した計画」（「今」の入れ替えを含む）を同じ旧 Shown_Plan に対する `scoreSchedule` の総費用で比べ、真に低い方（同点は K）。K が違法なら再生成。「今」の解除は物理的な例外ではなく、この比較の結果としてだけ起こる。反例（6 釜・釜 0 boiled・arms 1・L 5・同卓の 45 秒麺と別注文の大盛 45 秒麺）を固定 Example にし、両候補の合法性と総費用（現実装の再生成が 30 秒相当低い）を示す。
