...
14. **1 段目の「今」は絶対保護しない。「今」の入れ替えは費用改善の判断として明示する（2026-09-07・レビュー判断）。** 1 段目はまだ調理者に見せていない内部候補である。守る対象は (i) 実際に示した提案の安定性（保持候補 R と変更費用）と (ii) 完成した計画の開始可能性（最終の表示順の先頭が押せる）であって、1 段目で「今」に選んだ品目そのものではない。ゆえに、固定した「今」を残した再生成が計画順で成り立たないとき「物理的な例外として固定を外す」とは扱わない——反例：6 釜・釜 0 だけ boiled・arms 1・上げ間隔 5 秒で、同卓の 45 秒麺と別注文の大盛 45 秒麺。候補 K（1 段目の時刻を保ち釜だけ交換：同卓の麺は 30 秒後・大盛は今、空き釜）は共通の物理的ハード制約をすべて通る。現実装（同卓の麺を今へ繰り上げ、上げ窓の競合で大盛を 5 秒後へ）は総費用が 30 秒相当低いが、それは物理的に開始不能だからではなく別品目を繰り上げる業務上の選択である。**契約**：固定した「今」を残した再生成が成り立たない場面では、候補 K（1 段目の時刻を保ち、配分した釜だけを交換した計画）を組んで検証し、K が合法ならそれと再生成（「今」の入れ替えを含む）を同じ旧 Shown_Plan に対する総費用で比べ、真に低い方を採る（同点は K）。K が違法なときだけ再生成に落ちる。「今」を失う品目が在るのは、K が違法か、K より総費用が真に低い計画が在るときだけ。
