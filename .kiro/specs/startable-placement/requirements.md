...
8. THE 2 段目 SHALL 1 段目の各品目の `startAt` を下限として保持する（前倒ししない）。「今」の集合（`startAt ≤ now` の品目）は 1 段目と同じで、2 段目で増えも減りもしない。時刻は 1 段目以上（釜の組の制約で遅れ得る）
...
7. **「今」の集合の不変**：2 段目の `startAt ≤ now` の品目の集合は 1 段目と等しく、各品目の `startAt` は 1 段目以上
