...
5. **2 段の組み方。** 1 段目の「今」置く品目の集合と表示順を 2 段目に渡す形（品目 → 釜の対応表を `assignSlots` の強制候補として読ませるか、「今」配置を固定した接頭辞として尾部を組み直すか）。前回に忠実な計画と候補比較の計画（`plan-stability`）のどちらにも同じ 2 段を通す。2 段目で「今」の集合が変わらないこと（Startable_Slot はいずれも解放 `now` なので時刻は動かない）を性質にする。
