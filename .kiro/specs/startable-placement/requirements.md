...
7. **「今」の集合と時刻の保持（2026-09-07 改訂）**：2 段目の `startAt ≤ now` の品目の集合は 1 段目の集合を含み、「今」の品目の時刻は動かず、すべての「今」の品目は最終的な表示順の配分を受けて互いに素な釜を持つ（釜は反復で変わり得る）。復元された将来配置は 1 段目と同じ時刻。反例（同卓の先着 Short 60 秒・後着 Long 600 秒・釜 0 boiled・釜 1 だけ空き）で Short が釜 1・Long が釜 0 に配られ、Short の提案が出る
