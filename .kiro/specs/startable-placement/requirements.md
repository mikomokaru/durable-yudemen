...
14. **1 段目の「今」は絶対保護しない。「今」の入れ替えは費用改善の判断として明示する（2026-09-07・レビュー判断）。** 1 段目はまだ調理者に見せていない内部候補である。守る対象は (i) 実際に示した提案の安定性（保持候補 R と変更費用）と (ii) 完成した計画の開始可能性（最終の表示順の先頭が押せる）であって、1 段目で「今」に選んだ品目そのものではない。ゆえに、固定した「今」を残した再生成が計画順で成り立たないとき「物理的な例外として固定を外す」とは扱わない——反例：6 釜・釜 0 だけ boiled・arms 1・上げ間隔 5 秒で、同卓の 45 秒麺と別注文の大盛 45 秒麺。候補 K（1 段目の時刻を保ち釜だけ交換：同卓の麺は 30 秒後・大盛は今、空き釜）は共通の物理的ハード制約をすべて通る。現実装（同卓の麺を今へ繰り上げ、上げ窓の競合で大盛を 5 秒後へ）は待ち時間の勘定では 30 秒相当低い（実測・2026-09-07：同期の重み 0 で 1427 対 1457。既定の重みでは同卓の 2 品を 30 秒ずらす卓の遅れを払い、逆に K が 30 秒相当低い——1457 対 1487）が、どちらの向きでもそれは物理的に開始不能だからではなく別品目を繰り上げる業務上の選択である。**契約**：固定した「今」を残した再生成が成り立たない場面では、候補 K（1 段目の時刻を保ち、配分した釜だけを交換した計画）を組んで検証し、K が合法ならそれと再生成（「今」の入れ替えを含む）を同じ旧 Shown_Plan に対する総費用で比べ、真に低い方を採る（同点は K）。K が違法なときだけ再生成に落ちる。「今」を失う品目が在るのは、K が違法か、K より総費用が真に低い計画が在るときだけ。
15. **固定した「今」との同期と、再生成した batch 内部のまとまりは分ける（2026-09-07・レビュー判断）。** 再生成は固定した「今」の品目に残りの品目を揃えない（Short／Long の帰結・承認済み）。一方、再生成した品目どうしのまとまり（batch の pack／split・合流の規則・Property 1／16 の錨の一致）は、固定した品目を除いた集合について従来どおり成り立つ。記述も検査も分ける。
...
8. ~~THE 2 段目 SHALL 1 段目の各品目の `startAt` を下限として保持する~~（**2026-09-07 撤回**：下限は実装由来の規則で、合流分を窓が説明しない時刻へ押して `keepsAnchor` (d) を破った——場面 E。代わりに）THE 2 段目 SHALL 1 段目の将来配置（`startAt > now`）を**そのまま復元**し、固定した「今」配置と手前の一片で進めた表に対して物理的なハード制約で検証し、不正な一片だけをその位置で再生成する。再生成で新たに「今」になった品目が在れば、**既存の「今」も含めた集合全体を最終的な表示順で配り直し**、再び復元・検証する（保持するのは「今に選んだ品目とその時刻」であり、釜は変更不能にしない——後着の品目に先に空き釜を固定すると、後から「今」になった先着の品目が boiled の釜に落ちて押せず、後続も隠れる。内部の `pool` が尽きたことを厨房の空き不足と同一視しない）。継続する反復ごとに「今」の集合が必ず増えるので有限回で止まる。「今」の集合は縮まず、「今」の品目の時刻は動かず、すべての「今」の品目が最終的な表示順の配分を受ける。再配分後の合法性は別に検証する
...
1. **開始できる先頭**：任意の遷移の直後（再計画の時点）で、**最終的に「今」に選ばれた品目の集合**（1 段目の「今」と、2 段目の再生成で「今」になった品目・`startAt ≤ now`）のうち最終的な表示の順（`startAt` 昇順・同値は到着順）で最初の品目について、**採用済み接頭辞の予約と表示順で先行する配分を反映した解放表で、今割り当てられる Startable_Slot** がその slotSpan に足るなら、確定計画の推奨はその品目を Startable_Slot に `startAt ≤ now` で置き、表示（`liftGroups` → `slotSuggestions`）に先頭として現れる（Requirement 3.4 が開始を妨げる接頭辞を落とした後の接頭辞に対して）。**Timer の無い釜でも、採用済み接頭辞の将来配置がその釜を予約していれば（レビュー実走：採用済み B が Timer の無い釜 1 に開始 30・終了 90 で在り、1 段目の A は boiled の釜 0 に「今」——A を釜 1 へ移せば B と重複する）、今割り当てられる釜ではない**。そのときは AC 3.2 の排他による待ちに当たり、主張しない。1 段目が「今」を一つも選ばない計画（全員が待つ）でも何も主張しない（将来品目の繰上げはスコープ外）
7. **「今」の集合と時刻の保持（2026-09-07 改訂・判断 14 で限定）**：2 段目の `startAt ≤ now` の品目の集合は 1 段目の集合を含む——**ただし判断 14 の入れ替えを除く**：1 段目の「今」が完成した計画で「今」でなくなるのは、候補 K（1 段目の時刻を保ち釜だけ交換）が違法か、K より総費用（同じ旧 Shown_Plan に対する）が真に低い計画が在るときだけで、検査は K を実際に組んで合法性と総費用を比べる（完成後の他品目・釜を固定して時刻だけ戻す検査では判別できない）。「今」の品目の時刻は動かず、すべての「今」の品目は最終的な表示順の配分を受けて互いに素な釜を持つ（釜は反復で変わり得る）。復元された将来配置は 1 段目と同じ時刻。反例（同卓の先着 Short 60 秒・後着 Long 600 秒・釜 0 boiled・釜 1 だけ空き）で Short が釜 1・Long が釜 0 に配られ、Short の提案が出る
9. **再生成した品目どうしのまとまり（2026-09-07・判断 15）**：固定した「今」の品目を除いた再生成の集合について、batch の pack／split・合流の規則・錨の一致（Property 1／16）は従来どおり成り立つ。固定した品目との同期は主張しない
