...
5. THE 「今」置く品目への Startable_Slot の割当 SHALL 計画の順（卓の群の正準順序・群の中の並び）で行い、先頭群が先に受け取る
...
1. **開始できる先頭**：任意の状態で、Startable_Slot が先頭群の先頭の品目の slotSpan 以上在り、ハード制約がその品目を「今」置くことを許すなら、確定計画の推奨はその品目を Startable_Slot に `startAt ≤ now` で置き、表示（`liftGroups` → `slotSuggestions`）に先頭として現れる
2. **提案ゼロにならない**：「空き釜に開始時刻到来済みの推奨があるのに提案ゼロ」——Startable_Slot に置かれた `startAt ≤ now` の推奨が在るのに `slotSuggestions` が空——は起こらない（観測事実 8 の 24 品の対照を、完了順の昇順・降順の両方で、二周目以降まで検査）
...
6. **予測は変えない**：候補の開始が `now` より後の配置は、本 spec の前後で同じ釜に置かれる
...
3. **ゲートで Timer の在る釜への「今」配置を落とすか。** 判断 5 は落とさない（feasibility は解放表の契約のまま）。外部ソルバは自前解と同じ規則を通るので同じ配置を出す。手作りの計画だけが置き得る。推奨は落とさない（本 spec の関心は自前解の選択規則）。
