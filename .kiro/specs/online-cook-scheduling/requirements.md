...
3. THE Cook_Scheduling SHALL 次のハード制約を満たさない計画を feasible と認めない: (a) 同一 slot を同一時間帯に複数品目へ割り当てない、(b) 各時点の同時走行本数を slot 数以下に保つ、(c) 開始済み Timer の slot 割当・実効 `endTime` を変更しない、(d) 各品目に相異なる `slotSpan` 個の slot を割り当てる（釜容量は `slotSpan` の合計で数える。`lift-group-planning` で追補・ADR-0002）、(e) 同じ卓の走行中 Timer の錨に合流できる品目を、合流できない品目のために錨より後ろへ押し出さない（始めたまとまりを崩さない。`lift-group-planning` AC 1.11・ADR-0007）、(f) 店舗全体の上げ窓（長さ `liftIntervalSeconds` の半開区間）のうち**当該配置を含む**窓の負荷（上がる品目の `slotSpan` の合計。走行中の実効 `endTime` と計画順に手前の配置を含む）を `arms + HELPER_ARMS`（手伝いの分・定数 2）以下に保つ。含まない窓（走行中だけで既に超えている窓）は見ない。`arms` の超過は採点（Lift_Overflow）に委ねる（`lift-group-planning` 判断 20・AC 9.5・ADR-0009）
4. THE Cook_Scheduling SHALL 次をソフト制約（目的関数への劣後項）として扱う: (a) 同一 Table_Group の品目の提供時刻の近接、(b) 同一オーダーの複数品目の同時提供、(c) Slot_Affinity（関連品目間の slot 物理距離のうち許容距離を超過した分の最小化）、(d) ~~Arms_Overflow（同じ `serveAt` を持つ卓の成員——走行中を含む——の本数のうち `arms` を超えた分。重みは `max(0, w_table − 1)` から導く。`lift-group-planning` で追補・ADR-0002）~~ **改訂（`lift-group-planning` 判断 20・ADR-0009）**：Lift_Overflow（店舗全体の上げ窓で `arms` を超えた本数——上がる時刻を昇順に走査し、未割当の最早の時刻を起点に長さ `liftIntervalSeconds` の窓の負荷から `max(0, 負荷 − arms)` を足して窓の内側を割当済みにする一意な貪欲。重みは 1 本あたり `liftIntervalSeconds` 秒相当で、新しい重みは足さない）。「計画では arms はソフト」（ADR-0002）は保ち、`arms + HELPER_ARMS` の上限だけをハード制約 (f) に置く
...
> **確定済み（ユーザー確認）:** ソフト制約の織り込みは**整数重み付き和**とする（レキシコグラフィックは採らない。決定性のための整数演算と相性が良く、Plan_Unit ごとの分解が加算で素直に成立するため）。目的関数 = Σ Wait_Time(秒) + w_table × Σ(同一 Table_Group の提供時刻の最大差のうち許容幅 60 秒 超過分) + w_order × Σ(同一オーダーの提供時刻の最大差のうち許容幅 30 秒 超過分) + w_affinity × Σ(関連品目間の slot 物理距離のうち許容距離 超過分)。slot の物理距離は店舗レイアウト（slot のグリッド座標・サーバ権威設定）から算出する（距離尺度の具体は design で定める）。**3 項すべてを「許容幅からの超過分」の形に揃える**（ユーザー確定）——距離を生の値のまま計上すると、関連品目を同時に提供するには別 slot が必須（ハード制約 (a)）で距離が 0 になりえず、到達不能な下限が品目数に応じて底上げされ、主目的との比較を水増しするため。許容距離の既定は**斜め隣接まで**を許容する値とする（隣り合う釜ならペナルティ 0。具体値は距離尺度と同じ単位系で design が定める）。重みは w_table = 2・w_order = 3・w_affinity = 1（w_table / w_order は秒換算、w_affinity は距離換算のペナルティ係数）。主目的の集計単位は**品目単位**（オーダーと客の 1:1 対応が保証されない制約が存在しうるため）。品目単位の和とソフト制約 (b) の緊張は、w_order = 3 を最大に置き「同一オーダーはまとめる」を優先する形で対処する。ソフト制約の列挙は開いており（Introduction 参照）、追加が確定した場合は本 Requirement へ追補する。
>
> **改訂（`lift-group-planning` 判断 5・ADR-0001 / 0006）:** 上の卓同期項「最大差の許容幅 60 秒 超過分」は撤回し、目的関数は次の形になった——Σ Wait_Time(秒) + w_table × Σ Table_Lag + max(0, w_table − 1) × Σ Arms_Overflow + w_order × Σ(同一オーダーの最大差の 30 秒 超過分) + w_affinity × Σ(許容距離 超過分)。Table_Lag は卓の成員（未着手＋同じ卓の走行中 Timer。走行中は実効 `endTime` を `serveAt` とし動かせない）のうち最遅からの各成員の遅れの和で、秒換算は**切り上げ**（wait は切り捨て）。この形の最適点は群の全員が `max(走行中の錨, earliest の最大)` に揃う配置で、一致は制約ではなく採点の帰結になる。`tableSyncToleranceSeconds` は読む計算が無くなり撤去候補。また、未開始品目の Wait_Time（AC 3.2）は Boil_Sync による開始後の調整（±h_i）を織り込まない**近似**であり、開始済み品目（調整後の実効 `endTime` を用いる）との間に定義の非対称がある。改善判定は両計画を同じ近似の上で比較するため一貫する。
>
> **改訂（`lift-group-planning` 判断 20・ADR-0009）:** 上の `max(0, w_table − 1) × Σ Arms_Overflow` は撤回し、目的関数は次の形になった——Σ Wait_Time(秒) + w_table × Σ Table_Lag + w_order × Σ(同一オーダーの最大差の 30 秒 超過分) + w_affinity × Σ(許容距離 超過分) **+ Lift_Overflow（店舗全体・秒相当）**。Lift_Overflow は店舗全体の上げ表（走行中の実効 `endTime` と全配置の `serveAt`）から導く項で卓の内側に閉じないため、**総和 `total` にだけ足し、Plan_Unit ごとの部分和 `bySlice` には入れない**（Requirement 6.2(d) の「部分和の総和は全体値に一致する」の唯一の例外。段 1 の部分和比較は枝刈りとして働き、段 2 の総和比較が単調改善を担う）。走行中だけで超えている窓の超過も総和に載るが、同じ時点の比較には定数ゆえ効かない。`max(0, w_table − 1)` の導出は消えた。自前解は候補時刻（合流の規則）から `arms + HELPER_ARMS` の窓を満たす最初の時刻へ `serveAt` を後ろに動かし（`firstFit`）、同じ時刻に上げたい列が `arms` を超えて上限に収まる場合は pack（同じ窓）と split（`arms` 本と残り）を局所費用（Σ wait + w_table × Σ Table_Lag + ΔLift_Overflow）で比べる。`Placement` は合流先の錨を `anchor` として持ち（永続 v11）、窓で `serveAt` が後ろへ動いても合流の所属は変わらない。

> **改訂（`plan-stability` 判断 1〜8・ADR-0010・2026-09-06）:** 総和に **Change_Cost（店舗全体・秒相当）** を足し、目的関数は次の形になった——total = Σ bySlice + Lift_Overflow **+ Change_Cost**。`bySlice`（Plan_Unit ごとの部分和・上の 4 項）は変えない。Change_Cost は、前回配信対象として永続確定した提案（Shown_Plan・`TimerState.shownPlan`・永続 v12）と新しい計画の**対応する品目**（`externalOrderId` + `itemIndex`）の間で数える変更費用で、L = `liftIntervalSeconds` から重みを導き新しい設定は足さない——(a) 先頭の変更（比較の時点の now と遷移後の Timer 集合で両側から導いた Head から外れる）2L、(b) 釜の変更 L、(c) 前回同じ群だった組の分割・対応する組の `startAt` の順の逆転 それぞれ組ごとに L、(d) h_i を超える `startAt` の移動 跨いだ窓の数 × L / (k + 1) の床（k = 前回の `startAt` が今から何個目の窓か）。開始済み・キャンセル済み・新規の品目は数えず、未着手の品目の欠落にも費用を定めない（欠落は `isStale` と合成の尾部が閉じる）。Lift_Overflow と同じく卓の内側に閉じない項ゆえ **`total` にだけ足す**（上の「唯一の例外」は Lift_Overflow と Change_Cost の 2 項になる。段 1 の部分和比較は業務費用のまま、段 2 の総和比較に変更費用が乗る）。Acceptance_Gate の改善判定の基準は Committed_Plan のまま——外部解も同じ費用で採点され、前回と大きく違う計画で微小な改善を出す外部解は通りにくくなる。署名は `scoreSchedule(slices, pending, scoreContext: ScoreContext, params)` で、`ScoreContext { members, lifts, change: ChangeContext | null }`（`change` が null なら比較の相手なし・Change_Cost 0）。自前解は前回の釜を第一候補にし、前回のまとまり・先頭・配置を保つ分割を候補に足して局所費用（業務費用 + 変更費用の差分）で比べ、前回に忠実な計画と候補を比べた計画の 2 本のうち総費用が真に下がるときだけ後者を採る（`plan-stability` Requirement 3・design Component 5）。
...
> **CP-SAT モード限定の差分案（F-1・再確認対象・2026-09-09）：** [cpsat-planner-integration の R5.8・R9.12 と差分表](../cpsat-planner-integration/requirements.md) に、確定状態が不変なら `[RequestPlan]` または空列を許容する例外を明示する。承認後は本 AC の Persist 先頭という不変条件と冒頭の同設計制約に対し、その範囲だけ同 spec を優先する。状態変更列の Persist 先頭・唯一性、依存する保存の成功後の送出、RequestPlan の末尾配置は維持し、TS モードの契約は変更しない。
