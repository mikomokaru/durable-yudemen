...
`affinityToleranceDistance` の妥当域は **0 以上の整数**である。上限を持たない。理由は 2 つ。

1. **座標に上限が無いことと揃う。** 格子座標は下限 `GRID_COORDINATE_MIN = 0` だけを持ち、上限を置かない（台の増設を設定側で縛らない）。座標が無制限なら `slotDistance` も無制限に伸びる。距離の側を有限で止める理由がない。
2. **大きすぎる許容は害がない。** 許容距離を上げても affinity 項が 0 に潰れるだけで、それは `affinityWeight = 0` でも表せる状態である。上限で守るべき破綻がない。

帰結として、**どんなレイアウトでも全ペアの超過を 0 にできる許容距離が必ず存在する**（全ペアの最大 `slotDistance` を取ればよい）。上限があると、その値が上限を超えるレイアウト——たとえば slot 0 が (0,0)・slot 17 が (53,79) なら距離 `10×79 + 4×53 = 1002`——で「超過を 0 にできる設定が存在しない」状態が生まれていた。これは設計の意図ではない。Property 19 の前件（許容距離内の配置）が任意のレイアウトで作れることは、この帰結に依る。

**却下した案。**

- **上限を 1120 以上へ引き上げる。** 数の根拠が検証側の座標生成器（0〜40）になる。座標に上限が無い限り、どの有限値も理論上は超えられる。根拠のない定数を残す形ゆえ退けた。
- **座標に上限を入れ、許容距離の上限をそこから計算する。** 両方を有限にして整合させる案で、根拠のない定数は消える。だが「台の増設を設定側で縛らない」という既存方針の撤回になる。方針を保つほうを採った。
- **検証側で前件が作れない入力を除外する（`fc.pre`）。** production を触らずに済むが、「全ペアの超過を 0 にできない設定が存在する」状態は残る。検証の都合で設計の不整合を隠す形ゆえ退けた。
...
| `affinityToleranceDistance` | 14（斜め隣接） | 0 以上の整数（上限を置かない） |
| `unitOrigins` | `u → (4u, 0)` | 各座標が 0 以上の整数 |
| `slotOffsets` | 3 行 × 2 列（`j → (j % 2, ⌊j / 2⌋)`） | 各座標が 0 以上の整数・要素数 6 |

`affinityToleranceDistance` だけが上限を持たない。座標（`unitOrigins` / `slotOffsets`）に上限が無いことと揃えるためである。距離は座標から導出されるため、座標が無制限なら距離の許容幅も無制限でなければ「全ペアの超過を 0 にできる設定」が作れないレイアウトが生まれる。上限を外しても害はない——大きすぎる許容は affinity 項を 0 に潰すだけで、それは `affinityWeight = 0` と同義である（理由と却下案は上記「許容距離に上限を置かない」）。

#### 畳み込みの形が 1 個だけ変わる（実装への申し送り）

上限を外すと、`affinityToleranceDistance` の畳み込みは**下限のみ**になる。0 以上の整数はそのまま通し、非整数・非有限・負値を `DEFAULT_AFFINITY_TOLERANCE_DISTANCE = 14` へ畳む。`AFFINITY_TOLERANCE_DISTANCE_MAX` は削除する。

これにより、設定 9 個のうち 1 個だけ畳み込みの形が非対称になる。共通の芯 `toBoundedInteger`（下限・上限・既定を受け取り、不正は既定・範囲内はクランプ）は上限を前提にしているため、この 1 個には使えない。上限を省略可能にするか別の関数を立てるかは実装時の判断とする。ここに記すのは**形が変わる事実とその理由**だけである。

`AFFINITY_TOLERANCE_DISTANCE_MAX` は検証側の 3 ファイル（`tests/core/objective.property.test.ts`・`tests/core/scheduleScenes.ts`・`tests/core/store-config.property.test.ts`）が import しており、いずれも更新を要する。
...
client 側の扱いは変わらない。受信した値を表示・導出にのみ用い、変更要求は送らない（サーバ権威）。`unitOrigins` / `slotOffsets` は画面のスロット配置を物理配置に合わせる用途に将来使えるが、現時点で client は使わなくてよい（配信するが利用は強制しない）。**（2026-09-05 改訂）** `lift-group-display` が最初の用途を与えた——`ClientView.unitOrigins` / `slotOffsets` / `affinityToleranceDistance` として写し、domain へ移した `slotDistance`（`src/domain/store.ts`・engine の採点と同じ尺度）で釜の組を選ぶ。画面の配置には依然として用いない。
...
前件は任意のレイアウトで作れる。`affinityToleranceDistance` に上限が無いため、全ペアの最大 `slotDistance` を許容距離に取れば必ず前件を満たす設定が得られる（→ Data Models / 許容距離に上限を置かない）。上限があった間は、座標を広く振ったレイアウトで前件を満たす設定が存在せず、この Property は検証可能ではなかった。
