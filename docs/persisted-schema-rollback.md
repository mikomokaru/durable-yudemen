...
| v15 | `noodle-portions`（判断 4・10・2026-09-18） | `OrderItem.portions`（麺の玉数・0.5 刻み・必須）。釜数は `slotSpanOf(portions) = ⌈portions / 1.5⌉` の導出値で持たない。v8〜v14 の `slotSpan` は仮置きの玉数（`portions = slotSpan`）として読む（運用前ゆえ読み手に届かない前提） | `OrderItem.slotSpan`（釜数） | `version` を 14 にし、**各品目に `slotSpan = slotSpanOf(portions)` を書き戻す**。v14 の `reviveSlotSpan` は欠如を 1 へ畳むので、書き戻さなければ大盛（2 玉）が 1 釜として復元される。`portions` は v14 の reviver が読まないので放置してよい（次の `Persist` で消える）。v14 には玉数の概念が無く、札に玉数が出ない——それが v14 の正常動作である |
| v14 | `order-flow`（Orders 画面の卓の指定・2026-09-17） | `OrderItem.tableAssignedAt`（店が卓を決めた時刻・欠如は null）。非 null の品目は POS の後着で `tableId` を上書きされない | — | `version` を 13 にするだけ。v13 の `toOrderItem` は `tableAssignedAt` を読まないので放置してよい（次の `Persist` で消える）。**切戻しの注意**：店が決めた卓は v13 では「POS 申告の卓」と区別されず、次の POS の後着で POS の値に戻る——v13 には店の判断の概念が無いので、それが v13 の正常動作である。走行中 Timer の卓（`Timer.orderItem.tableId`）は v13 の reviver がそのまま読むので、店が書き換えた錨は残る |
