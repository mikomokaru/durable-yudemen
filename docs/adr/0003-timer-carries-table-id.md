...
> **（2026-09-08 改訂・ADR-0013）** 本文の「PendingOrder は消費され、走行中 Timer がどの卓のものかは状態から辿れなくなる」は `order-lifecycle` で前提が変わった——品目（`OrderItem`）は開始で消費されず生涯を通じて残り、状態は Timer の参照から導く。それでも `tableId` を Timer の `orderItem` の内側に持つ判断は残る：計画の錨は**調理を開始した時点の卓**で採点し（後着で品目の卓が移っても走行中 Timer は旧卓の群として茹でている事実）、参照先の品目から卓を引き直せば計画が後着で動く。「開始しても PendingOrder を消さず『開始済み』の印を付ける案は採らない」の理由（Timer から導ける事実を二重に持たない）は、状態の enum を保存しない判断としてそのまま引き継がれた。wire については Consequences の 2 点目を再改訂した。
...
- `TimerFact`（client ワイヤ）には出さない。`lift-group-display` は一度 `orderItem` をワイヤに出した（判断 16・群の開始の判定のため）が、実運用の差し戻しで群の所属と合流の錨を `CookRecommendation.group` / `anchor` として engine が運ぶ形（ADR-0008・同 spec 判断 20）になり、読み手が無くなったので撤去した。engine の `Ordered.orderItem.tableId` は永続 v10 の事実として残る。走行中カードに卓を表示する必要が生じたとき、その spec が改めて判断する。 **（2026-09-08 再改訂・ADR-0013）** その判断は `order-lifecycle` が下した——読み手（走行中カードの番号・卓・品名・`lift-order-numbering`）が現れたので、`TimerFact` は `orderItem: { externalOrderId, itemIndex } | null`（参照の鍵だけ）を運ぶ。**`tableId` は今も wire に出さない**——client は参照で品目（`OrderItem`＝最新の注文情報）を引いて卓を読み、engine の `Ordered.orderItem.tableId`（開始時点の卓）は計画の錨の出所として engine 専用のまま。
