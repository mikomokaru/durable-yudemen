...
- **受理時に弾く**: 受理時の値域窓（`pos-order-ingress` AC 8.12〜8.15：受理時刻の 2 時間前より古い Record は保留）は既に在るが、受理時だけの判定では**受理後の期限切れ**を扱えない——受理の時点で生きていた注文は後から死ぬ。受理（後着の全置換・重複排除の材料）を変えれば冪等の契約に触れる。受理は一行も変えない。
...
- ~~正本の集合は伸び続ける（`pending-order-expiry` 未決 1 は (a) 残す）。絞った値が正なので観測されない。Persist のサイズが実機で問題になったら、確定結果の `Persist` に相乗りして期限切れを落とす形を別 spec で判断する（自分では `Persist` を起こさない）。~~ **→ ADR-0014（2026-09-09）が置き換えた。** 正本は `ORDER_ITEM_LIMIT` = 4096 件で有界化され、超えた分は `compareArrival` の古い順に忘れられる。ただし**期限ではなく件数**で、しかも**別の遷移としてではなく集合の構築点**（`upsertOrder` の出口と `migrate`）で行う——本 ADR の判断（期限は述語であって状態を書き換えない）はそのままで、`truncateOrderItems` は `isLive` を呼ばない。
