...
待ち行列の品目（`PendingOrder`）と Timer は別の実体で、開始で品目を消費して Timer に写し、完了・Cancel で Timer を消していた。Timer が調理中を語る唯一の事実であり、engine の Timer は `orderItem { externalOrderId, itemIndex, tableId }` を持つが、**client に配信する Timer 表現（`TimerFact`）には注文への参照が無かった**。帰結が三つ。走行中カードは卓も品名も知らず、茹で上がる順の番号を注文で切れない（`lift-order-numbering` の要望）。厨房の Cancel は Timer を消すだけで、消費済みの品目は失われ POS の後着でしか戻らない。後着の受理には「生きた Timer が指す品目は置換の結果から除く」という規則が要り、A を調理中に同じ注文 {A, B} が再送されるだけで正本が {B} に置き換わって A の参照先が消える。目標は、走行中カードが注文・卓・品名を参照で読めること、番号を注文で切れること、厨房の Cancel が品目を待ち行列へ戻すこと。
...
> **改訂（ADR-0014・2026-09-09）:** 正本は `ORDER_ITEM_LIMIT` = 4096 件で有界化された。整理は「別の機能」ではなく**集合の構築点に作り込まれた上限**（`upsertOrder` の出口と `migrate`）であり、超えた分は `compareArrival` の古い順に落ちる。**生きた Timer の参照先も削除対象から外さない**——守る述語を一つ入れれば「上限を超えているのに落とせない」場面が生まれるためで、落ちた参照先は本 ADR が既に一つに畳んである経路（`orderItemOf` が null・`complete` / `cancel` は品目に何も書かず Timer を閉じる）へ落ちる。ゆえに**本 ADR が v13 の注文 Timer について述べる参照整合（生きた Timer の参照先は集合に在り、配信に含まれる）は、上限を超えた場合には成り立たない**——参照が解けない Timer は、旧版由来・アドホックに限らず起こりうる。扱いは同じ経路のままで、新しい経路も新しい拒否事由も足していない。
...
- 正本の保持量は増え、整理は別の機能（**→ ADR-0014 が改めた：整理は別機能ではなく構築点の上限**）。snapshot の `orderItems` は「期限内 ∨ 参照先」で全件送る限り、`done` を別配列に分けても配信量は減らない——期限 2 時間の内側に限られることで足りるとして始め、実測で問題になれば `done` を送らない方向で別途判断する。
