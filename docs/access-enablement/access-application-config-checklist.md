...
- [ ] 認証対象外経路のための **bypass 用アプリ定義**は許すが、その個数は**店舗数に依存しない定数個**であること
  - Cloudflare Access のセルフホストアプリはパス包含で定義され、単一アプリ定義内に「このパスだけ除外」の切り欠きを作れない。ポリシーの Include / Exclude に指定できるのは identity・network の属性（email・IdP グループ・IP・国・service token 等）であってパスではないため、既存アプリへ Bypass ポリシーを足すとホスト全体が素通しになる。ゆえに対象外経路は**別アプリ定義 ＋ 全員 Bypass ポリシー**で実現する
  - 1 つのセルフホストアプリには**複数の hostname / path を登録できる**。同一アプリ内の全ドメインは同じポリシーを共有するため、ポリシーが同一（全員 Bypass）の対象外経路は **1 つの bypass 用アプリにまとめられる**
...
- [ ] `/pos/records`（POS_Ingress・pos-order-ingress 由来）
  - POS からの Record 取り込みを受ける機械向け経路。ブラウザセッションを持たない上流ゆえ、Access のログイン誘導（302）は取り込みをそのまま止める
  - `ORDER_INGRESS_TOKEN`（`ADMIN_TOKEN` とは別 secret）の Bearer 定数時間照合を `src/worker-auth.ts` の `isOrderIngressAuthorized` が担う。`/admin/*` と同じ理由で Access を二重に被せない
  - パス指定は**ワイルドカードを付けない**（`pos/records` のみ）。`/pos/*` は将来この配下へ足す経路まで無条件に開いてしまう
...
> 実現方式: 上記 3 経路を**同一の bypass 用アプリ定義**に登録し、**全員 Bypass ポリシー**（Everyone を Bypass）を 1 つ割り当てる。ポリシーが同一ゆえアプリを分ける必要がない（§1-1）。これは店舗別ポリシーではない（要件 10.5）。

> **Action は Bypass であること。Allow ではない。** `Allow ＋ Everyone` は「誰でも通す」ではなく「誰でも**認証すれば**通す」であり、ログイン誘導の 302 は出続ける。ポリシーを付け忘れた場合（0 件）も全要求が拒否され同じく 302 になる。いずれも見かけの症状が「設定が効いていない」と区別しづらい。
>
> **どのアプリが要求を処理したかは 302 の `meta` JWT の `aud` で判る。** ログイン先 URL の `kid`（= 処理したアプリの aud）が既存の認証アプリの `POLICY_AUD` なら、パス定義が当たっていない。bypass 用アプリの aud なら、パス定義は当たっており原因はポリシー側にある。この 2 つを切り分けてから直す。

> 検証（2026-09-01 実測）: `/pos/records`・`/admin/*`・`/favicon.svg` を 1 つの bypass 用アプリへまとめ、全員 Bypass を割り当てた構成で、当該 3 経路の 302 が消え、`/`・`/entry/stores`・`/s/{storeId}/` の保護は維持された。bypass の対象を 3 経路に限定できることを確認済み。
...
- [ ] 除外経路 = `/admin/*`・`/pos/records`・既存アセット経路（例: `/favicon.svg`）を bypass 用アプリ定義 ＋ 全員 Bypass ポリシーで実現（本番と同一・個数は店舗数非依存の定数個）
...
| 除外経路（bypass） | `/admin/*`・`/pos/records`・既存アセット | 同左 | ○ 一致 |
