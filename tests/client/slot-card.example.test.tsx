...
describe("走行中・茹で上がりのバッジ——番号のマーカーと品名・卓の語（lift-order-numbering R2.1〜2.4）", () => {
  ...
  it("品目が持つ札は、バッジの可視の語にも aria-label にも出る", () => {
    ...
    const shown = badge();
    expect(shown.getAttribute("aria-label")).toBe("Boiling 4b: 特味噌ネギ中盛 · Table 12 · Thin 1");
    expect(shown.textContent).toBe("4bThin 1特味噌ネギ中盛 · 12");
  });
  ...
  it("走行中：マーカーは上がり順（クラスタ番号＋枝・aria-hidden のチップ）、可視の語は卓を数だけにし、読み上げは `Table 卓` を残す", () => {
    ...
    expect(shown.getAttribute("aria-label")).toBe("Boiling 4b: プレ塩中盛 · Table 12 · Thin 1");
    ...
    expect(shown.textContent).toBe("4bThin 1プレ塩中盛 · 12");
  ...
  it("走行中：同時に上がる注文が 1 つでも枝は出す（単独のクラスタは `3a`・表記の形を揃える）", () => {
    ...
    expect(shown.getAttribute("aria-label")).toBe("Boiling 3a: プレ塩中盛 · Table 12 · Thin 1");
  });
  ...
  it("走行中：卓を持たない品目は品名だけ（Table の語を出さない）", () => {
    ...
    expect(badge().getAttribute("aria-label")).toBe("Boiling 1a: プレ塩中盛 · Thin 1");
    expect(badge().textContent).toBe("1aThin 1プレ塩中盛");
  });
  ...
  it("茹で上がり：マーカーは ✓ のまま（番号は無い）、語は品名と卓", () => {
    ...
    const shown = badge();
    expect(shown.getAttribute("aria-label")).toBe("Ready: プレ塩中盛 · Table 12 · Thin 1");
    expect(shown.querySelector("[aria-hidden]")?.textContent).toBe("✓");
    // 可視の語は走行中と同じ規則で卓を数だけにする（読み上げは `Table 12` のまま）。
    expect(shown.textContent).toBe("✓Thin 1プレ塩中盛 · 12");
  });
