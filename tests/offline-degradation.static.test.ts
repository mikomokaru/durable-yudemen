...
//
//   (a) core 無変更・shell 一点 — src/core/ にオフライン劣化由来の差分（auto-response / heartbeat /
//       client 結合）が無く、変更が src/client/ と src/shell/store-timer-do.ts の
//       setWebSocketAutoResponse 一点のみであること（要件12.1）。
//   (b) ワイヤ形式不変 — src/shared/messages.ts の既存メッセージ種別のみを用い、新種別・新フィールド
...
const MESSAGES_FILE = "src/shared/messages.ts";

/** 心拍フレームの確定値（素の文字列。ワイヤ型ではない）。 */
const HEARTBEAT_FILE = "src/shared/heartbeat.ts";
...
describe("(a) core 無変更・shell は auto-response 一点のみ（要件12.1）", () => {
  it("src/core のファイル集合が確定集合と一致する（オフライン劣化で core にファイルを足していない）", () => {
    expect(collectSourceFiles("src/core")).toEqual([...EXPECTED_CORE_FILES]);
  });
  ...
  it("store-timer-do.ts は心拍値を src/shared/heartbeat から取り込み、client を import しない", () => {
    const withStrings = readCodeWithStrings(SHELL_FILE);
    expect(withStrings, "shell が heartbeat の心拍値を import していない").toMatch(
      /import\s*\{[^}]*\bPING_REQUEST\b[^}]*\bPONG_RESPONSE\b[^}]*\}\s*from\s+["'][^"']*\/shared\/heartbeat["']/,
    );
...
describe("(b) src/shared/messages の既存ワイヤ形式のみを用いる（要件12.2）", () => {
  ...
  it("client はワイヤ型（ClientMessage / ServerMessage / WireTimer）を再定義しない", () => {
    for (const file of CLIENT_FILES) {
      const code = readBareCode(file);
      expect(code, `${file} がワイヤ型を再定義している`).not.toMatch(
        /\b(?:type|interface)\s+(?:ClientMessage|ServerMessage|WireTimer)\b/,
      );
    }
  });
});
