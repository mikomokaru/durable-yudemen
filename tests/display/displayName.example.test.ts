...
import { displayName } from "../../src/client/components/queueDisplay";
...
describe("麺量", () => {
  ...
  it("`中盛` / `大盛` / `半玉` は語のまま出し、区切りを置かない", () => {
    ...
    expect(displayName(item({ ...base, sizeName: "中盛" }))).toBe("特味噌ネギ中盛");
    expect(displayName(item({ ...base, sizeName: "大盛" }))).toBe("特味噌ネギ大盛");
    expect(displayName(item({ ...base, sizeName: "半玉" }))).toBe("特味噌ネギ半玉");
  });
