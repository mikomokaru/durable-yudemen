...
// 採点パラメータ（重み・許容幅・距離・上げの間隔・レイアウト）はイデアの主張対象そのものに入っていないが、POS の対応表 2 枚は主張できる（合成対象で
// ある）。ここが供給するのは「当該フィクスチャが主張しない」場合の既定であり、対応表を関心事とするテストは
// 自身で値を与えてこの供給を上書きする。
//
// 供給は 2 段に分かれる。schedulingDefaults は採点パラメータ（ScheduleParams ちょうど 11 値）だけを返し、
...
/** 採点パラメータ（重み 3・arms・許容調整割合・許容幅 2・距離 1・上げの間隔・レイアウト 2）の既定。unitOrigins は unitCount に依存する。 */
...
/**
 * StoreConfig の残余（arms・toleranceRatio を除く採点パラメータ 9 値 ＋ POS の対応表 2 枚）を既定値で満たした部分。
 *
