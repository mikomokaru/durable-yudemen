...
const genScenario = genYearAndMonth.chain(({ year, month }) => {
  ...
  return fc.record({
    ...
    generated: fc.uniqueArray(fc.record({
      identity: fc.integer({ min: 1, max: 1_000_000 }),
      observedOffset: fc.integer({ min: 0, max: emptyMonthStart - nextMonthStart - 1 }),
      lag: fc.constantFrom<number | null>(null, 0, FIFTEEN_MINUTES - 1, FIFTEEN_MINUTES, FIFTEEN_MINUTES + 1),
...
function rawArrivals(logical: readonly LogicalArrival[], orderSeed: number): Arrival[] {
  return logical
    .flatMap((arrival) => Array.from({ length: arrival.duplicateCount }, (_, duplicateIndex): Arrival => ({
      record: { ...arrival.record } as OperationRecord,
      firstObservedAt: arrival.firstObservedAt + duplicateIndex,
      firstSnowflakeAt: arrival.firstSnowflakeAt === null || arrival.firstSnowflakeAt === undefined
        ? null
  ...
  it("重複除外後の初回観測を UTC 月へ一度だけ所属させ、15分率と空月の対象外を算出する", () => {
    fc.assert(
      fc.property(genScenario, ({ year, month, generated, orderSeed }) => {
        ...
        expect(new Set(result.map(({ utcMonth: resultMonth }) => resultMonth))).toEqual(new Set(utcMonths));
        expect(arrivals.length).toBeGreaterThan(logical.length);

        for (const resultMonth of result) {
          const expectedPopulation = logical.filter(({ firstObservedAt }) =>
            utcMonth(firstObservedAt) === resultMonth.utcMonth
          );
          const expectedWithinFifteenMinutes = expectedPopulation.filter((arrival) => {
            if (arrival.firstSnowflakeAt === null || arrival.firstSnowflakeAt === undefined) return false;
            const elapsed = arrival.firstSnowflakeAt - arrival.firstObservedAt;
            return elapsed >= 0 && elapsed <= FIFTEEN_MINUTES;
          }).length;
          const expectedRate = expectedPopulation.length === 0
            ? null
        ...
        const thresholdMonth = result.find(({ utcMonth: resultMonth }) => resultMonth === utcMonth(monthStart));
        ...
        const emptyMonth = result.find(({ utcMonth: resultMonth }) => resultMonth === utcMonth(emptyMonthStart));
