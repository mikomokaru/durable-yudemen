...
import { parseOperationLines, type OperationLineFailure } from "../../src/operation-history/codec";
...
const KNOWN_ATTRIBUTES = [...COMMON_ATTRIBUTES, "startTime", "endTime", "boiledAt"] as const;
...
function commonMembers(
  ...
  return COMMON_ATTRIBUTES.map(
    (name) =>
      [name, overrides[name] ?? (name === "operationKind" ? "completed" : seed[name])] as const,
  );
}
function validLine(seed: CommonSeed): GeneratedLine {
  ...
  return {
    line: jsonObject(commonMembers(seed, { operationKind: "completed" })),
    expected: { ok: true, record },
  };
}
...
const genDuplicateLine = genCommon.chain((seed) =>
  fc.constantFrom(...KNOWN_ATTRIBUTES).map((duplicate): GeneratedLine => {
    ...
    const members: JsonMember[] = COMMON_ATTRIBUTES.filter((name) => name !== missing).map(
...
const genLineMatrix = fc
  ...
  .chain(([valid, invalidJson, duplicate, missing, disallowed, type, value, trailingValid]) =>
    fc
      .shuffledSubarray([valid, invalidJson, duplicate, missing, disallowed, type, value], {
        minLength: 7,
        maxLength: 7,
      })
...
describe("Property 5: 行 parser の失敗分類と継続性", () => {
  ...
  it("六種の失敗を優先分類し、各行の結果と1始まり行番号を入力順に保つ", () => {
    fc.assert(
      fc.property(genLineMatrix, (rows) => {
        const expected = rows.map(({ expected: lineExpected }, index) =>
          lineExpected.ok ? lineExpected : { ...lineExpected, lineNumber: index + 1 },
        );
