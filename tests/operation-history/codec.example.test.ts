...
describe("canonical Operation Record printer", () => {
  it("全kindの既知属性を固定順で出力する", () => {
    expect(
      records.map((record) => Object.keys(JSON.parse(printCanonicalOperationLine(record)))),
    ).toEqual([
      [
        "storeId",
        "timerId",
  ...
  it("標準JSON表記を使い、未知・自然人・採番・導出属性を出力しない", () => {
    ...
    expect(line).toBe(
      JSON.stringify({
        storeId: '店"舗\n\\',
        timerId: common.timerId,
        operationKind: "completed",
        eventTime: common.eventTime,
        slotIds: common.slotIds,
        noodleType: common.noodleType,
        firmness: common.firmness,
      }),
    );
  ...
  it("複数recordをLF一個で連結しrecordとslotIdsの順序を保つ", () => {
    ...
    expect(printCanonicalOperationLines(selected)).toBe(
      selected.map(printCanonicalOperationLine).join("\n"),
    );
...
describe("Operation Record line parser", () => {
  ...
  it("未知属性とその重複を無視し、既知属性とslotIds順を保つ", () => {
    const line =
      '{"unknown":{"eventTime":"ignored"},"storeId":"store-1","unknown":false,"timerId":"timer-1","operationKind":"completed","eventTime":1700000000000,"slotIds":["slot-2","slot-1"],"noodleType":"Thin","firmness":"normal"}';
    expect(parseOperationLines(line)).toEqual([{ ok: true, record: records[3] }]);
  });

  it("escape表現を含む同じ既知属性の重複を拒否する", () => {
    const line =
      '{"storeId":"store-1","store\\u0049d":"store-2","timerId":"timer-1","operationKind":"completed","eventTime":1700000000000,"slotIds":["slot-2"],"noodleType":"Thin","firmness":"normal"}';
  ...
  it("kind別の必須属性と不許可な既知属性を拒否する", () => {
    const missingEndTime =
      '{"storeId":"store-1","timerId":"timer-1","operationKind":"adjusted","eventTime":1,"slotIds":["slot-1"],"noodleType":"Thin","firmness":"normal"}';
    const completedWithEndTime =
      '{"storeId":"store-1","timerId":"timer-1","operationKind":"completed","eventTime":1,"slotIds":["slot-1"],"noodleType":"Thin","firmness":"normal","endTime":2}';
  ...
  it("既知属性の型制約と値制約を区別して拒否する", () => {
    const wrongType =
      '{"storeId":"store-1","timerId":"timer-1","operationKind":"completed","eventTime":"1","slotIds":["slot-1"],"noodleType":"Thin","firmness":"normal"}';
    const wrongValue =
      '{"storeId":"INVALID","timerId":"timer-1","operationKind":"completed","eventTime":0,"slotIds":[],"noodleType":"","firmness":"unknown"}';
...
describe("Operation History Codec example / edge coverage", () => {
  it("全kindのcanonical UTF-8 bytesを固定する", () => {
    ...
    expect(records.map((record) => encode(printCanonicalOperationLine(record)))).toEqual(
      goldenLines.map(encode),
    );
  });
  ...
  it("正整数timestampの下限と安全整数上限を受理し、0・負数・小数を拒否する", () => {
    ...
    const invalidLines = [
      ...
      printCanonicalOperationLine(boundaryRecord).replace(
        `"endTime":${Number.MAX_SAFE_INTEGER}`,
        '"endTime":1.5',
      ),
    ];

    expect(parseOperationLines(printCanonicalOperationLine(boundaryRecord))).toEqual([
      { ok: true, record: boundaryRecord },
    ]);
  ...
  it("全kindでparse(print(record))とprint(parse(canonical line))をexampleとして固定する", () => {
    for (const record of records) {
      ...
      expect(parsed).toEqual({ ok: true, record });
      if (parsed?.ok) {
        expect([...new TextEncoder().encode(printCanonicalOperationLine(parsed.record))]).toEqual([
