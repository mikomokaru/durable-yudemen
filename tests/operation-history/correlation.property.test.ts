...
function primaryKey(record: OperationRecord): string {
  return JSON.stringify([record.storeId, record.timerId, record.operationKind, record.eventTime]);
}
        ...
        for (const candidate of candidates) {
          ...
          expect(
            candidate.records.every(
              (record) => primaryKey(record) === primaryKey(group[0]!.record),
            ),
          ).toBe(true);
