...
import { parseOperationLines, printCanonicalOperationLine } from "../../src/operation-history/codec";
import type { OperationRecord } from "../../src/operation-history/record";
import { nonEmpty } from "../nonEmpty";

const NUM_RUNS = 200;
const KNOWN_ATTRIBUTES = new Set([
  "storeId", "timerId", "operationKind", "eventTime", "slotIds",
  "noodleType", "firmness", "startTime", "endTime", "boiledAt",
]);
...
describe("Property 4: 未知属性に対する既知意味の不変性", () => {
  ...
  it("任意の未知属性を追加しても既知属性値と slotIds 順を保つ", () => {
    fc.assert(fc.property(genRecord, genUnknownName, fc.jsonValue(), (record, unknownName, unknownValue) => {
