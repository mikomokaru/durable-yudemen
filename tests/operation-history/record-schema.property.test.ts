...
const NUM_RUNS = 200;
const COMMON_KEYS = ["storeId", "timerId", "operationKind", "eventTime", "slotIds", "noodleType", "firmness"] as const;
const KEYS_BY_KIND = {
...
const genStoreId = fc.array(fc.constantFrom(..."abcdefghijklmnopqrstuvwxyz0123456789-"), { minLength: 1, maxLength: 64 }).map((chars) => chars.join(""));
const genObservation = fc.record({
  storeId: genStoreId,
  eventTime: fc.integer({ min: 1, max: 2_000_000_000_000 }),
  eventKind: fc.constantFrom<OperationObservation["eventKind"]>("Start", "Adjust", "Complete", "Cancel", "AlarmFired", "Reconcile"),
  timerId: genText,
  ...
  const timer = createTimer({ id: seed.timerId as TimerId, slotIds: nonEmpty(seed.slotIds.map((id) => id as SlotId)), noodleType: seed.noodleType as NoodleType, firmness: seed.firmness, startTime: seed.startTime as EpochMillis, endTime: (seed.startTime + seed.duration) as EpochMillis, seq: 7 });
  const empty = { ...EMPTY_STATE, timers: [], nextSeq: 8 } as const;
  const populated = { ...EMPTY_STATE, timers: [timer], nextSeq: 8 } as const;
  if (seed.eventKind === "Start") return { storeId: seed.storeId, eventTime: seed.eventTime, eventKind: seed.eventKind, before: empty, after: populated };
  if (seed.eventKind === "Complete" || seed.eventKind === "Cancel") return { storeId: seed.storeId, eventTime: seed.eventTime, eventKind: seed.eventKind, before: populated, after: empty };
  const changed = createTimer({ ...timer, firmness: seed.firmness === "normal" ? "hard" : "normal", boiledAt: seed.eventTime as EpochMillis });
  return { storeId: seed.storeId, eventTime: seed.eventTime, eventKind: seed.eventKind, before: populated, after: { ...EMPTY_STATE, timers: [changed], nextSeq: 8 } };
});
  ...
  it("kind 別の既知属性と値制約だけを持ち、自然人属性・連番・追加導出値を持たない", () => {
    fc.assert(
      fc.property(genObservation, (observation) => {
        ...
        expect(Object.keys(record).toSorted()).toEqual([...KEYS_BY_KIND[record.operationKind]].toSorted());
